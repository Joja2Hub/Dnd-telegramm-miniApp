...
class DamageResult:
    ...
    pain_before: int
    pain_after: int
    temp_hp_before: int = 0
    temp_hp_after: int = 0
    temp_hp_damage: int = 0
...
def compute_damage(
    ...
    *,
    ...
    allowed_locations = {loc for loc, _ in legacy.INJURY_LOCATION_WEIGHTS}
    forced_location = str(forced_location or "").strip() or None
    if forced_location not in allowed_locations:
        forced_location = None
    forced_severity = str(forced_severity or "").strip() or None
    if forced_severity not in {"light", "medium", "heavy"}:
        forced_severity = None
    debug_forced_injury = bool(force_injury and forced_location and forced_severity)

    hp_before = int(char["current_hp"])
    temp_before = max(0, int(char.get("temp_hp", 0) or 0))
    ...
    if remaining_damage > 0 and temp_before > 0:
        temp_damage = min(temp_before, remaining_damage)
        temp_after = max(0, temp_before - temp_damage)
        remaining_damage = max(0, remaining_damage - temp_damage)
        char = db.update_character_fields(character_id, temp_hp=temp_after)

    hp_damage = remaining_damage
    armor_damage = 0
    armor_after = armor_before

    if armor_enabled and armor_mode != "ignore" and remaining_damage > 0:
        if armor_mode == "normal":
            armor_damage = min(armor_before, remaining_damage)
            hp_damage = max(0, remaining_damage - armor_damage)
    ...
    res = DamageResult(
        ...
        temp_hp_after=temp_after,
        temp_hp_damage=temp_damage,
    ...
    if (damage <= 0 or hp_damage <= 0) and not debug_forced_injury:
        return res
    ...
    injury_damage_basis = max(1, hp_damage if hp_damage > 0 else damage)
    if debug_forced_injury:
        location = forced_location or "torso"
        res.location = location
        res.injury_chance = 1.0
        res.injury_roll = 0.0
    else:
        location = legacy.weighted_choice(rng, legacy.INJURY_LOCATION_WEIGHTS)
        res.location = location
        chance = float(legacy.injury_chance(int(char["max_hp_base"]), injury_damage_basis, location, rng))
        roll = rng.random()
    ...
    severity = forced_severity if debug_forced_injury else legacy._severity_from_damage(int(char["max_hp_base"]), injury_damage_basis)
    ...
    hp_loss = dynamic_injury_hp_loss(int(char["max_hp_base"]), injury_damage_basis, severity, rng)
