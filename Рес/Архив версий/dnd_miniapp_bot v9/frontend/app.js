...
function renderCampaign() {
  ...
  const tabs = role === 'master'
    ? ['characters','combat', ...(MAPS_FEATURE_ENABLED ? ['maps'] : []), 'requests','achievements','settings','journal']
...
function renderCampaignSettingsCard() {
  ...
  const editBlock = isMaster ? `<div class="campaign-edit-box">
      ...
      <button class="ok" onclick="saveCampaignSettings()">Сохранить название и иконку</button>
    </div>` : '';
...
async function saveCampaignSettings() {
  try {
    ...
    if (!name) throw new Error('Введите название кампании');
    const out = await api(`/api/campaigns/${state.currentCampaignId}`, {method:'PATCH', body:{name, emoji}});
...
function renderMasterSettings() {
  return `<div class="stack">${renderSparkManagementCard()}${renderCampaignSettingsCard()}${renderGeneratorSettingsCard()}</div>`;
}
