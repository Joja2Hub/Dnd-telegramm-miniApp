...
class EffectEngine {
      ...
      pushRain(palette, options={}) {
        this.particles.push({
          kind:'rain',
          x:this.rand(options.x0 ?? 8, options.x1 ?? this.w-8),
          y:this.rand(options.y0 ?? -28, options.y1 ?? 4),
          vy:this.rand(options.vy0 || 190, options.vy1 || 340),
          len:this.rand(options.len0 || 8, options.len1 || 19),
          life:this.rand(options.life0 || .9, options.life1 || 1.3),
          age:0,
          color:this.pick(palette),
      ...
      pushSnowParticle(palette, options={}) {
        this.particles.push({
          kind:'snow',
          x:this.rand(options.x0 ?? 8, options.x1 ?? this.w-8),
          y:this.rand(options.y0 ?? -18, options.y1 ?? 2),
          vx:this.rand(options.vx0 ?? -18, options.vx1 ?? 18),
          vy:this.rand(options.vy0 ?? 34, options.vy1 ?? 86),
          life:this.rand(options.life0 || 1.8, options.life1 || 3.8),
          age:0,
          phase:Math.random()*Math.PI*2,
      ...
      pushFallingShard(palette, options={}) {
        this.particles.push({
          kind:'shard',
          x:this.rand(8,this.w-8),
          y:this.rand(-20,6),
          vx:this.rand(options.vx0 ?? -18, options.vx1 ?? 18),
          vy:this.rand(options.vy0 ?? 70, options.vy1 ?? 145),
          rot:Math.random()*Math.PI*2,
      ...
      pushNeonRain(options={}) {
        this.particles.push({
          kind:'rain',
          x:this.rand(options.x0 ?? this.cx-76, options.x1 ?? this.cx+76),
          y:this.rand(options.y0 ?? -22, options.y1 ?? 4),
          vy:this.rand(options.vy0 || 190, options.vy1 || 330),
          len:this.rand(options.len0 || 7, options.len1 || 16),
          life:this.rand(options.life0 || .75, options.life1 || 1.18),
          age:0,
      ...
      pushMatrixGlyph(palette, glyphs, options={}) {
        ...
        this.particles.push({
          kind:'matrixGlyph',
          x,
          y:this.cy - clipR - this.rand(2, 14),
      ...
      spawnOne() {
        switch(this.type){
          ...
          case 'ashFall':
            if (Math.random()<.7) return this.particles.push({kind:'fallDot', x:this.rand(8,this.w-8), y:this.rand(-18,0), vy:this.rand(38,82), vx:this.rand(-10,10), life:this.rand(1.4,2.3), age:0, size:this.rand(2.4,4.2), color:this.pick(['#a8a29e','#d6d3d1','#fb923c']), shadow:7});
          ...
          case 'snowDust':
            if (Math.random()<.7) return this.particles.push({kind:'snow', x:this.rand(8,this.w-8), y:this.rand(-18,2), vx:this.rand(-12,12), vy:this.rand(28,70), life:this.rand(2.0,3.8), age:0, size:this.rand(2.4,4.8), color:'#f8fafc'});
          ...
          case 'sootRain':
            if (Math.random()<.86) return this.particles.push({kind:'fallDot', x:this.rand(this.cx-76,this.cx+76), y:this.rand(-18,0), vy:this.rand(36,78), vx:this.rand(-6,6), life:this.rand(1.4,2.8), age:0, size:this.rand(1.5,3.2), color:this.pick(['#020617','#111827','#292524','#44403c']), shadow:4});
...
function desiredCanvasEffectState(el) {
  ...
  // В карточках магазина/кастомизации эффект не запускается сам по наведению: клик открывает модалку.
  if (wrapper?.closest?.('.frame-choice') && !wrapper?.closest?.('.cosmetic-preview-modal')) return false;
  if (el.classList?.contains('avatar')) return true;
...
function effectAnchorBaseSize(el) {
  if (!el || !el.isConnected) return 24;
  const measure = (node) => {
    if (!node) return 0;
    const rect = node.getBoundingClientRect?.();
    return Math.max(rect?.width || 0, rect?.height || 0, node.offsetWidth || 0, node.offsetHeight || 0);
  };
  if (el.classList?.contains('fx-avatar-shell')) {
    return Math.max(measure(el.querySelector('.avatar')), 24);
  }
  if (el.classList?.contains('effect-preview-layer')) {
    return Math.max(measure(el.closest('.effect-choice-preview')?.querySelector('.frame-demo.plain, .effect-avatar-sample')), 32);
  }
  if (el.classList?.contains('frame-demo') && el.classList.contains('effect-demo')) {
    return Math.max(measure(el), 40);
  }
  if (el.classList?.contains('avatar')) {
    return Math.max(measure(el.querySelector('.avatar-photo, .avatar-initial')), measure(el), 24);
  }
  const shell = el.closest?.('.avatar-shell');
  if (shell) {
    return Math.max(measure(shell.querySelector('.avatar')), 24);
  }
  return Math.max(measure(el), 24);
}
function sizeCanvasEffectHost(rec) {
  if (!rec?.el?.isConnected) return;
  const base = effectAnchorBaseSize(rec.el);
  if (rec.el.classList?.contains('fx-avatar-shell')) {
    // HTML preview system in the real app:
    // the canvas gets generous transparent padding, but the engine still draws
    // by the real avatar diameter through data-effect-anchor-base.
    const style = getComputedStyle(rec.el);
    const rawPad = Number(style.getPropertyValue('--fx-canvas-scale') || 2.35);
    const padScale = Math.max(2.05, Math.min(2.75, Number.isFinite(rawPad) ? rawPad : 2.35));
    const size = Math.round(Math.max(48, base * padScale));
    rec.canvas.dataset.effectAnchorBase = String(Math.round(base));
    rec.canvas.style.width = `${size}px`;
    rec.canvas.style.height = `${size}px`;
    rec.canvas.style.left = '50%';
    rec.canvas.style.top = '50%';
  ...
  const rawScale = Number(css.getPropertyValue('--effect-canvas-scale') || wrapperCss?.getPropertyValue('--effect-canvas-scale') || 1.44);
  const hostScale = Math.max(1.34, Math.min(1.74, Number.isFinite(rawScale) ? rawScale : 1.44));
  const size = Math.round(Math.max(44, base * hostScale));
  rec.canvas.dataset.effectAnchorBase = String(Math.round(base));
  rec.canvas.style.width = `${size}px`;
  rec.canvas.style.height = `${size}px`;
  rec.canvas.style.left = '50%';
...
function bindEffectActivation() {
  document.querySelectorAll('.fx-avatar-shell, .avatar.cosmetic-effect, .avatar-shell, .effect-choice-preview, .frame-demo.effect-demo').forEach(el => {
    ...
    el.addEventListener('click', ev => {
      // В карточках магазина/кастомизации клик должен открывать предпросмотр, а не просто мигать эффектом.
      if (el.closest?.('.frame-choice, .shop-buy-card, .locked-preview-card, .cosmetic-preview-modal')) return;
      toggleEffectActivation(el);
      ev.stopPropagation();
    }, {passive:false});
    el.addEventListener('touchstart', () => {
      if (el.closest?.('.frame-choice, .shop-buy-card, .locked-preview-card, .cosmetic-preview-modal')) return;
      const target = getEffectActivationTarget(el);
      if (target && !target.classList.contains('effect-force-active')) {
        document.querySelectorAll('.effect-force-active').forEach(x => { if (x !== target) x.classList.remove('effect-force-active'); });
        target.classList.add('effect-force-active');
        scheduleCanvasEffectsFrame();
      }
    }, {passive:true});
  });
...
async function openUserProfile(telegramId, focusCharacterId=0) {
  ...
  try {
    ...
    // v62: если профиль открыт из окна отряда, показываем именно нажатого
    // персонажа текущей кампании, а не последнего созданного персонажа игрока.
    ...
    const campList = camps.map(c => `<span class="tag-pill ${Number(main.campaign_id) === Number(c.id) ? 'selected' : ''}">${esc(c.name)}</span>`).join('') || '<span class="muted">Кампаний нет.</span>';
...
function renderParty() {
  ...
  return `<div class="card stack"><div class="name">🧭 Отряд</div><div class="muted">Нажми на персонажа, чтобы открыть профиль игрока.</div><div class="party-grid">${team.map(ch => `<button class="party-card party-card-button" style="--char-color:${esc(ch.color || '#72a7ff')}" onclick="openUserProfile(${Number(ch.telegram_user_id || 0)}, ${Number(ch.id || 0)})">${avatarCircle(ch, 'md')}<div class="party-main"><b>${esc(ch.name)}</b>${characterTagPill(ch)}<span class="${hpClass(ch)}">HP ${esc(ch.current_hp)}/${esc(ch.current_max_hp)}</span></div><span class="badge ${ch.injury_count ? 'danger':'ok'}">травмы: ${ch.injury_count || 'нет'}</span></button>`).join('')}</div></div>`;
}
...
function effectChoiceCard(charId, e, available, selected) {
  ...
  const demo = effectPreviewHtml(e);
  if (!available) {
...
function shopItemCard(kind, item, unlocked) {
  ...
  if (kind === 'effect') preview = effectPreviewHtml(item);
  if (kind === 'tag') preview = String(item.category || '') === 'tag_shape' ? tagShapePreview(item, 'Пример') : tagTextPreview(item, defaultTagShapeId());
  const action = unlocked ? '<span class="shop-status owned">куплено</span>' : isUnique ? '<span class="shop-status locked">только за ачивку</span>' : `<button class="small ok" onclick="event.stopPropagation(); buyShopItem('${kind}','${esc(item.id)}')">Купить за ${priceLabel(item)}</button>`;
...
function shopItemAvailable(kind, item) {
  ...
  if (kind === 'frame') return displayRarity(item) === 'common' || unlockedCosmeticSet().has(String(item.id));
  if (kind === 'effect') return String(item.id) === 'effect_none' || String(item.rarity || 'common') === 'common' || unlockedEffectSet().has(String(item.id));
  return false;
}
...
function shopActionButton(kind, item, charId=null) {
  ...
  if (available) {
    ...
    return `<button class="ok" onclick="event.stopPropagation(); applyShopItem('${kind}','${esc(item.id)}', ${charId ? Number(charId) : 'null'})">${selected ? 'Применено' : 'Применить'}</button>`;
  }
  if (rarity === 'unique' || item.rarity === 'unique' || item.purchasable === 0) {
    return '';
  }
  return `<button class="ok" onclick="event.stopPropagation(); buyShopItem('${kind}','${esc(item.id)}')">Купить за ${priceLabel(item)}</button>`;
}
...
function effectPreviewHtml(e, extraClass='') {
  const id = String(e?.id || '');
  const cls = effectClassFor(id);
  return `<div class="effect-choice-preview ${esc(extraClass)}"><span class="fx-avatar-shell md fx-preview-shell effect-force-active ${esc(cls)}" data-effect-id="${esc(id)}"><span class="avatar md avatar-empty effect-avatar-sample" style="--char-color:#ff6b8a"><span class="avatar-initial">○</span></span></span></div>`;
}
...
function showCosmeticPreviewModal(kind, item, locked=false, charId=null) {
  ...
  const preview = kind === 'frame' ? framePreviewHtml(item, 'modal-frame-preview-active') : effectPreviewHtml(item, 'modal-effect-preview-active');
...
function showLockedCosmetic(id, charId=null) {
  const c = cosmeticById(id);
  showCosmeticPreviewModal('frame', c || {id, name:'Рамка закрыта', rarity:'legendary'}, true, charId);
}

function showLockedEffect(id, charId=null) {
  const e = effectById(id);
  showCosmeticPreviewModal('effect', e || {id, name:'Эффект закрыт', rarity:'legendary'}, true, charId);
}
...
function renderMasterAchievements() {
  ...
  return `<div class="stack achievements-screen">
    ...
    <div class="card stack"><div class="name">🎖 Глобальная библиотека достижений</div><div class="muted">Шаблоны больше не привязаны к конкретной кампании: их можно выдавать игрокам из любой кампании.</div>${list}</div>
  </div>`;
}
