...
COSMETIC_LIBRARY.extend([
    ...
    {'id': 'ashen-bone', 'name': 'Пепельная кость', 'description': 'Костяная рамка для мрачных и боевых образов.', 'rarity': 'common', 'category': 'unique', 'css_class': 'frame-ashen-bone', 'emoji': '▫️', 'sort_order': 620},
    ...
    {'id': 'crystal-lattice', 'name': 'Кристальная решётка', 'description': 'Яркая кристаллическая рамка с магическим оттенком.', 'rarity': 'legendary', 'category': 'unique', 'css_class': 'frame-crystal-lattice', 'emoji': '🌟', 'sort_order': 780},
...
class Database:
    ...
    def delete_achievement(self, achievement_id: int, campaign_id: int | None = None) -> bool:
        ...
        if not ach:
            return False
        # v61: global achievement library; campaign_id is no longer used to block deletion.
    ...
    def grant_achievement(self, achievement_id: int, character_id: int, master_tg_id: int, master_comment: str = '') -> dict[str, Any]:
        ...
        # v61: achievement templates are global. The grant belongs to the character's campaign,
        # not to the campaign where the achievement template was originally created.
        cur = self.conn.execute(
            """
            ...
            """,
            (int(achievement_id), int(tg_id), int(ch.get("campaign_id") or 0), int(character_id), int(master_tg_id), master_comment.strip(), utc_now()),
        )
