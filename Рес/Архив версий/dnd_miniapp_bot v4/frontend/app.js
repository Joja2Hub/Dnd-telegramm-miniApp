...
class EffectEngine {
      ...
      resize() {
        ...
        this.w = rect.width || 258;
        this.h = rect.height || 258;
        this.cx = this.w / 2;
        this.cy = this.h / 2;
      }
...
function activationWrapperForEffectTarget(el) {
  return el?.closest?.('.effect-choice-preview, .avatar-shell, .frame-demo.effect-demo') || el;
}
...
function sizeCanvasEffectHost(rec) {
  ...
  const wrapperBox = wrapper && wrapper !== rec.el ? wrapper.getBoundingClientRect() : null;
  const base = Math.max(
    box.width || 0,
    box.height || 0,
    wrapperBox?.width || 0,
  ...
  const rawScale = Number(css.getPropertyValue('--effect-canvas-scale') || wrapperCss?.getPropertyValue('--effect-canvas-scale') || 1.82);
  const scale = Math.max(1.15, Math.min(2.45, Number.isFinite(rawScale) ? rawScale : 1.82));
...
function setupCanvasEffects() {
  document.querySelectorAll('.avatar.cosmetic-effect[data-effect-id], .effect-preview-layer[data-effect-id], .frame-demo.effect-demo[data-effect-id]').forEach(el => {
...
function getEffectActivationTarget(el) {
  ...
  if (el.classList?.contains('effect-choice-preview')) return el.querySelector('.effect-preview-layer');
  if (el.classList?.contains('avatar') && el.classList.contains('cosmetic-effect')) return el;
  if (el.classList?.contains('avatar-shell')) return el.querySelector('.avatar.cosmetic-effect');
  return el.querySelector?.('.avatar.cosmetic-effect, .effect-preview-layer, .frame-demo.effect-demo') || null;
}
...
function bindEffectActivation() {
  document.querySelectorAll('.avatar.cosmetic-effect, .avatar-shell, .effect-choice-preview, .frame-demo.effect-demo').forEach(el => {
...
function avatarCircle(ch, size='md') {
  ...
  const effectAttr = effect ? ` data-effect-id="${esc(effect)}"` : '';
  const avatarInner = avatarSrc
    ? `<span class="avatar ${size}${frameClass}${effectClass}"${effectAttr} style="--char-color:${esc(color)}"><img loading="lazy" class="avatar-photo" src="${esc(avatarSrc)}" alt=""></span>`
    : `<span class="avatar ${size} avatar-empty${frameClass}${effectClass}"${effectAttr} style="--char-color:${esc(color)}"><span class="avatar-initial">${esc(initial)}</span></span>`;
...
function effectChoiceCard(charId, e, available, selected) {
  ...
  const demo = `<div class="effect-choice-preview"><span class="frame-demo plain">○</span><span class="effect-preview-layer ${esc(cls)}" data-effect-id="${esc(e.id)}"></span></div>`;
  if (!available) {
...
function shopItemCard(kind, item, unlocked) {
  ...
  if (kind === 'effect') preview = `<div class="effect-choice-preview"><span class="frame-demo plain">○</span><span class="effect-preview-layer ${esc(effectClassFor(item.id))}" data-effect-id="${esc(item.id)}"></span></div>`;
...
function effectPreviewHtml(e, extraClass='') {
  ...
  return `<div class="effect-choice-preview ${esc(extraClass)}"><span class="frame-demo plain">○</span><span class="effect-preview-layer effect-force-active ${esc(cls)}" data-effect-id="${esc(id)}"></span></div>`;
}

function showCosmeticPreviewModal(kind, item, locked=false) {
  if (!item) return;
  ...
  showModal(`<div class="modal-card compact unique-modal locked-custom-modal cosmetic-preview-modal"><button class="modal-close" onclick="closeModal()">×</button>
    ...
    <div class="row modal-actions">${shopActionButton(kind, item)}<button class="secondary" onclick="closeModal()">Назад</button></div>
  </div>`);
}

function openShopPreview(kind, id) {
  if (kind === 'frame') return showCosmeticPreviewModal('frame', cosmeticById(id), false);
  if (kind === 'effect') return showCosmeticPreviewModal('effect', effectById(id), false);
  showToast('Раздел тэгов добавится позже', 'warn');
}

function showLockedCosmetic(id) {
  const c = cosmeticById(id);
  showCosmeticPreviewModal('frame', c || {id, name:'Рамка закрыта', rarity:'legendary'}, true);
}

function showLockedEffect(id) {
  const e = effectById(id);
  showCosmeticPreviewModal('effect', e || {id, name:'Эффект закрыт', rarity:'legendary'}, true);
}
...
function previewAvatarFile(inputId, targetId) {
  ...
  const effectClass = effect ? ` cosmetic-effect ${effectClassFor(effect)}` : '';
  const effectAttr = effect ? ` data-effect-id="${esc(effect)}"` : '';
  target.innerHTML = `<span class="avatar xl${frameClass}${effectClass}"${effectAttr} style="--char-color:${esc(ch.color || '#72a7ff')}"><img class="avatar-photo" src="${esc(url)}" alt=""></span>`;
}
