...
ImageKind = Literal["avatar", "achievement", "frame", "map"]
...
def _center_frame_hole(canvas: Image.Image) -> Image.Image:
    ...
    if best_count < (w * h * 0.03):
        # Some uploaded frames are not closed rings, so the inner hole is connected to
        # the outside transparency. In that case center by visible alpha bounding box.
        bbox = alpha.getbbox()
        if not bbox:
            return canvas
        lx, ty, rx, by = bbox
...
def _fit_square(img: Image.Image, px: int, *, contain: bool) -> Image.Image:
    ...
    if contain:
        # For uploaded avatar frames we preserve the exact uploaded canvas.
        # The master aligns scale/offset in the preview before saving; automatic
        # hole-centering would make the saved asset differ from the preview and
        # would visually reset the chosen scale/position on the player's avatar.
...
def _fit_map(img: Image.Image, px: int) -> Image.Image:
    img = img.copy()
    img.thumbnail((px, px), Image.Resampling.LANCZOS)
    return img
...
def optimize_upload(
    ...
    *,
    ...
    folder = {"avatar": "avatars", "achievement": "achievements", "frame": "frames", "map": "maps"}[kind]
    root = Path(upload_root) / folder
    thumbs = root / "thumbs"
    root.mkdir(parents=True, exist_ok=True)
    thumbs.mkdir(parents=True, exist_ok=True)

    # For frames keep the whole transparent ornament; for avatars/icons crop to square; maps preserve aspect ratio.
    if kind == "map":
        main_img = _fit_map(img.copy(), main_px)
        thumb_img = _fit_map(img.copy(), thumb_px)
    else:
        contain = kind == "frame"
