...
class AchievementCreateIn(BaseModel):
    ...
    custom_tag_style: str = Field(default="tag-custom-gold", max_length=80)
...
def create_app(db: Database, settings: Settings, bot: Any | None = None) -> FastAPI:
    ...
    async def create_campaign(data: CreateCampaignIn, user: TelegramUser = Depends(get_current_user)) -> dict[str, Any]:
        c = db.create_campaign(data.name, user.id, injuries_enabled=data.injuries_enabled, armor_enabled=data.armor_enabled, weapons_enabled=data.weapons_enabled)
    ...
    async def campaign_state(
        ...
        return {
            ...
            "achievement_templates": db.list_achievements(campaign_id),
            ...
            "inventories": {str(ch["id"]): db.list_inventory(int(ch["id"])) for ch in (db.list_characters(campaign_id) if role == "master" else ([player_character] if player_character else []))},
        }
    ...
    async def _apply_damage_and_notify(request: Request, ch: dict[str, Any], damage: int, *, armor_mode: str, title: str) -> dict[str, Any]:
        ...
        payload = {"damage": damage, "armor_mode": armor_mode, "character_before": before, "character_after": res.character}
    ...
    async def player_edit_self(
        ...
        if "custom_tag" in updates:
            ...
            if tag and tag != "tag_none":
                cosmetic_tag = db.get_tag(tag)
                if not cosmetic_tag:
                    raise HTTPException(400, "Неизвестный тэг")
                if not db.has_tag_unlocked(effective_player_id, tag):
                    raise HTTPException(403, "Этот тэг ещё не открыт")
        if not updates:
            return {"character": ch}
        new_ch = db.update_character_fields(character_id, **updates)
    ...
    async def upload_avatar(character_id: int, file: UploadFile = File(...), user: TelegramUser = Depends(get_current_user)) -> dict[str, Any]:
        ch = _character_or_404(db, character_id)
    ...
    async def upload_achievement_icon(campaign_id: int, file: UploadFile = File(...), user: TelegramUser = Depends(get_current_user)) -> dict[str, Any]:
        _require_master(db, campaign_id, user)
        content_type = (file.content_type or "").lower()
        if content_type not in {"image/jpeg", "image/png", "image/webp", "image/gif"}:
    ...
    async def create_achievement(campaign_id: int, data: AchievementCreateIn, user: TelegramUser = Depends(get_current_user)) -> dict[str, Any]:
        ...
        if custom_tag_name:
            ...
            style = data.custom_tag_style if data.custom_tag_style in allowed_styles else "tag-custom-gold"
        ...
        return {"achievement": ach, "achievements": db.list_achievements(campaign_id)}
    ...
    async def delete_achievement(campaign_id: int, achievement_id: int, user: TelegramUser = Depends(get_current_user)) -> dict[str, Any]:
        ...
        if not ach or int(ach.get("campaign_id") or 0) != int(campaign_id):
            raise HTTPException(404, "Достижение не найдено")
        ok = db.delete_achievement(achievement_id, campaign_id)
        if not ok:
        ...
        return {"message": "Достижение удалено", "achievements": db.list_achievements(campaign_id)}
    ...
    async def grant_achievement(campaign_id: int, achievement_id: int, data: AchievementGrantIn, request: Request, user: TelegramUser = Depends(get_current_user)) -> dict[str, Any]:
        _require_master(db, campaign_id, user)
        ach = db.get_achievement(achievement_id)
        if not ach or int(ach["campaign_id"]) != int(campaign_id):
            raise HTTPException(404, "Достижение не найдено")
    ...
    async def add_inventory_shell_stock(item_id: int, data: ShellStockCreateIn, user: TelegramUser = Depends(get_current_user), x_dev_view_character_id: str | None = Header(default=None, alias="X-Dev-View-Character-Id")) -> dict[str, Any]:
        ...
        if not item or item.get("item_type") != "weapon":
            raise HTTPException(404, "Оружие не найдено")
        if item.get("reload_type") != "shell":
            raise HTTPException(400, "У этого оружия магазинная перезарядка. Добавь магазин.")
        ch = _character_or_404(db, int(item["character_id"]))
        role = _role(db, int(ch["campaign_id"]), user.id)
        dev_character = _dev_impersonated_character(db, settings, int(ch["campaign_id"]), user, x_dev_view_character_id)
        if role != "master" and int(ch.get("telegram_user_id") or 0) != int(user.id) and not (dev_character and int(dev_character["id"]) == int(ch["id"])):
        ...
        db.log(int(ch["campaign_id"]), int(ch["id"]), "inventory", f"{ch['name']}: добавлена стопка патронов для {item['name']}", {"shell_stock": stock, "item_id": item_id})
    ...
    async def inventory_refill_request(item_id: int, data: RefillRequestIn, user: TelegramUser = Depends(get_current_user), x_dev_view_character_id: str | None = Header(default=None, alias="X-Dev-View-Character-Id")) -> dict[str, Any]:
        ...
        _ensure_no_open_player_request(db, int(ch["campaign_id"]), effective_player_id)
        try:
            req = db.create_inventory_request(int(ch["campaign_id"]), int(ch["id"]), int(item_id), effective_player_id, "refill_magazine", {"magazine_id": data.magazine_id, "amount": data.amount})
        except ValueError as exc:
            raise HTTPException(400, str(exc))
    ...
    async def purchase_shop_item(data: ShopPurchaseIn, user: TelegramUser = Depends(get_current_user)) -> dict[str, Any]:
        try:
            out = db.purchase_cosmetic_item(user.id, data.item_type, data.item_id)
        except Exception as e:
            raise HTTPException(400, str(e))
        return {
            **out,
            "unlocked_cosmetic_ids": db.list_unlocked_cosmetic_ids(user.id),
            "unlocked_effect_ids": db.list_unlocked_effect_ids(user.id),
            "unlocked_tag_ids": db.list_unlocked_tag_ids(user.id),
            "currency_balance": db.get_currency_balance(user.id),
            "currency_transactions": db.list_currency_transactions(user.id, 20),
        }
    ...
    async def grant_currency(campaign_id: int, data: CurrencyGrantIn, request: Request, user: TelegramUser = Depends(get_current_user)) -> dict[str, Any]:
        ...
        if not tg_id:
            raise HTTPException(400, "Персонаж ещё не привязан к игроку")
        balance = db.add_currency(int(tg_id), int(data.amount), reason=data.comment or "Выдано мастером", source='master', source_id=campaign_id)
        ...
        db.log(campaign_id, int(ch["id"]), "currency", f"{ch['name']}: {sign}{data.amount} искр", {"amount": data.amount, "comment": data.comment, "balance": balance})
