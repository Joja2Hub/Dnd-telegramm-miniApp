...
function uniqueTags() { return cosmeticTags().filter(c => c.category !== 'base'); }
...
function restoreUiScroll(){ setTimeout(()=>{
  ...
  const nav=document.querySelector('.bottom-nav');
  if(nav && Number.isFinite(state.bottomNavScroll)) nav.scrollLeft = state.bottomNavScroll;
  if(nav) nav.addEventListener('scroll', () => { state.bottomNavScroll = nav.scrollLeft || 0; localStorage.setItem('bottom_nav_scroll', String(state.bottomNavScroll)); }, {passive:true});
}, 0); }
...
function setupPullToRefresh() {
  ...
  document.addEventListener('touchstart', (e) => {
    if (!['achievements','settings'].includes(state.tab) && window.scrollY === 0) { startY = e.touches?.[0]?.clientY || 0; pulling = true; }
  }, {passive:true});
  document.addEventListener('touchmove', (e) => {
    if (!pulling) return;
    const dy = (e.touches?.[0]?.clientY || 0) - startY;
    const ind = document.getElementById('pullIndicator');
    if (ind && dy > 35) ind.classList.add('show');
  }, {passive:true});
  document.addEventListener('touchend', async (e) => {
    ...
    if (dy > 70 && !userIsEditing() && !['achievements','settings'].includes(state.tab)) {
      try { await refreshAll(); showToast('Обновлено', 'info'); } catch (_) {}
    }
  }, {passive:true});
}
...
async function createCampaign() {
  try {
    ...
    const out = await api('/api/campaigns', {method:'POST', body:{name, injuries_enabled:checked('newInj'), armor_enabled:checked('newArmor')}});
...
function navLabel(id, role='player') {
  return ({characters: role === 'master' ? 'Персонажи' : 'Персонаж', party:'Отряд', combat:'Бой', requests:'Заявки', generators:'Генераторы', achievements:'Достижения', journal:'Журнал', settings:'Настройки'})[id] || id;
}
function navIcon(id) {
  const icons = {
    characters:'🧙',
...
function renderCampaign() {
  ...
  const tabs = role === 'master'
    ? ['characters','combat','requests','generators','achievements','settings','journal']
    : ['characters','party','combat','achievements','settings'];
...
function setTab(t) { rememberUiScroll(); state.tab=t; renderCampaign(); }
...
function avatarCircle(ch, size='md') {
  ...
  const imageFrameClass = frameAsset ? ' image-frame' : '';
  const overlay = frameAsset ? `<img loading="lazy" class="avatar-frame-overlay" src="${esc(frameAsset)}" alt="">` : '';
  if (avatarSrc) {
    return `<span class="avatar ${size}${frameClass}${effectClass}${imageFrameClass}" style="--char-color:${esc(color)}"><img loading="lazy" class="avatar-photo" src="${esc(avatarSrc)}" alt="">${overlay}</span>`;
  }
  return `<span class="avatar ${size} avatar-empty${frameClass}${effectClass}${imageFrameClass}" style="--char-color:${esc(color)}"><span class="avatar-initial">${esc(initial)}</span>${overlay}</span>`;
}
...
function renderRequests() {
  ...
  if (!reqs.length) return '<div class="card muted">Открытых заявок нет.</div>';
  return `<div class="grid">${reqs.map(r => `<div class="card stack"><div class="name">${esc(r.character?.name || 'Персонаж')}</div><div class="pill">Тип: ${esc(reqLabel(r))}</div>${requestExtra(r)}<div class="row"><button class="ok" onclick="decideReq(${r.id},true)">Подтвердить</button><button class="danger" onclick="decideReq(${r.id},false)">Отклонить</button></div></div>`).join('')}</div>`;
}
...
function openImageModal(src, title='Изображение') {
  ...
  wrap.innerHTML = `<div class="modal-card image-view-modal"><button class="modal-close" onclick="closeImageModal()">×</button><div class="name">${esc(title)}</div><div class="image-view-box"><img class="full-quality-image" loading="eager" src="${esc(src)}" alt=""></div><button class="secondary" onclick="closeImageModal()">Закрыть</button></div>`;
...
function renderPlayerCharacter() {
  ...
  return `<div class="card stack player-card" style="--char-color:${esc(color)}">
    ...
    <div class="muted">Чтобы запросить стабилизацию или полное лечение травмы, нажми на саму травму.</div><div id="playerOut"></div>
  </div>`;
}
...
function renderCampaignSettingsCard() {
  ...
  return `<details class="card stack campaign-settings-card custom-collapse" ${sectionAttrs('campaign-info-settings')}>
    ...
    <div class="custom-collapse-body stack">
      <div class="muted">Роль: ${esc(role)} · травмы: ${c.injuries_enabled?'включены':'выключены'} · броня: ${c.armor_enabled?'включена':'выключена'}</div>
...
function renderPlayerSettings() {
  ...
  return `<div class="stack settings-page"><div class="card stack settings-card" style="--char-color:${esc(ch.color || '#72a7ff')}">
    <div class="player-hero profile-settings-hero">${avatarCircle(ch, 'lg')}<div><div class="name">⚙️ Настройки персонажа</div><div class="muted">Имя, цвет, аватарка, рамка, эффекты и тэг.</div><div class="currency-pill big">✦ ${currencyBalance()} искр</div></div></div>
    ...
    <div class="avatar-upload-block">
      ...
      <p class="muted">Выбери изображение, настрой масштаб и положение, затем сохрани.</p>
  ...
  </div>${renderCampaignSettingsCard()}</div>`;
}
...
function renderUniqueCustomizationBlock(ch) {
  ...
  const frameIsAvailable = c => c.category === 'base' || unlocked.has(c.id);
  const effectIsAvailable = e => e.category === 'base' || unlockedEffects.has(e.id);
  const tagIsAvailable = t => t.category === 'base' || unlockedTags.has(t.id);
...
function renderShopBlock(ch) {
  return `<details class="custom-section shop-section custom-collapse" ${sectionAttrs('player-shop')}><summary><div class="unique-custom-head"><span>🛒</span><div><b>Магазин косметики</b><small>Покупай рамки, эффекты и тэги за искры. Уникальное не продаётся.</small></div><span class="currency-pill">✦ ${currencyBalance()}</span></div></summary><div class="custom-collapse-body shop-tabs">
...
function shopItemsHtml(kind, items) {
  ...
  return rarityGroupHtml('shop', items, item => shopItemCard(kind, item, unlocked.has(item.id) || item.category==='base'));
}
...
function openAvatarCropModal(characterId, inputId) {
  ...
  avatarCropState = {characterId, file, url, scale: 1.15, x: 0, y: 0};
  showModal(`<div class="modal-card avatar-crop-modal"><button class="modal-close" onclick="closeModal()">×</button><div class="name">🖼️ Настрой аватарку</div><p class="muted">Подвинь изображение и выбери масштаб, как в Telegram. В круг попадёт область внутри рамки.</p><div class="avatar-crop-stage"><div class="avatar-crop-circle"><img id="avatarCropImg" src="${esc(url)}" alt=""></div></div><label>Масштаб<input id="avatarCropScale" type="range" min="0.6" max="2.5" step="0.01" value="1.15" oninput="updateAvatarCropPreview()"></label><label>Сдвиг по горизонтали<input id="avatarCropX" type="range" min="-160" max="160" step="1" value="0" oninput="updateAvatarCropPreview()"></label><label>Сдвиг по вертикали<input id="avatarCropY" type="range" min="-160" max="160" step="1" value="0" oninput="updateAvatarCropPreview()"></label><div class="row modal-actions"><button class="ok" onclick="saveCroppedAvatar()">Сохранить аватарку</button><button class="secondary" onclick="closeModal()">Отмена</button></div></div>`);
  setTimeout(updateAvatarCropPreview, 30);
}
function updateAvatarCropPreview() {
  const img = document.getElementById('avatarCropImg');
  if (!img || !avatarCropState) return;
  const scale = Number(val('avatarCropScale') || 1.15);
  const x = Number(val('avatarCropX') || 0);
  const y = Number(val('avatarCropY') || 0);
  avatarCropState.scale = scale; avatarCropState.x = x; avatarCropState.y = y;
...
async function saveCroppedAvatar() {
  ...
  try {
    ...
    ctx.fillStyle = 'transparent'; ctx.clearRect(0,0,size,size);
    ...
    <details class="card stack achievement-create" ${sectionAttrs('master-ach-create')}><summary><span>🏆</span><b>Создать достижение</b><small>PNG/JPG-иконка, описание, тэг и визуальная награда</small></summary>
      <div class="grid two achievement-form">
        ...
        <div class="wide achievement-rewards-grid">
          ...
          <div class="reward-picker-box"><div class="reward-picker-head"><b>Награда-тэг</b><button class="secondary small" onclick="openRewardPicker('tag')">Выбрать</button></div><div id="achTagRewardPreview" class="reward-empty">Без тэга</div></div>
...
function openRewardPicker(kind) {
  ...
  const items = sortByRarity(isFrame ? baseCosmetics().concat(uniqueCosmetics()) : isEffect ? baseEffects().filter(e=>e.id!=='effect_none').concat(uniqueEffects()) : baseTags().filter(t=>t.id!=='tag_none').concat(uniqueTags()));
  ...
  showModal(`<div class="modal-card reward-shop-modal"><button class="modal-close" onclick="closeModal()">×</button><div class="name">${isFrame ? '🖼️ Выбор рамки-награды' : isEffect ? '✨ Выбор эффекта-награды' : '🏷️ Выбор тэга-награды'}</div><p class="muted">Это как магазин наград: выбери, что откроется игроку после получения достижения.</p><div class="reward-shop-grid ${isFrame ? 'frame-shop-grid' : 'effect-shop-grid'}">${cards}</div></div>`);
}
...
function openCustomFrameModal() {
  showModal(`<div class="modal-card custom-frame-modal"><button class="modal-close" onclick="closeModal()">×</button><div class="name">🖼️ Добавить кастомную рамку</div><p class="muted">Загрузи PNG/WebP/GIF рамку с прозрачным центром. Она появится в библиотеке уникальных рамок. Редкость загружаемых рамок по умолчанию — <b>уникальная</b>.</p><label>Название рамки<input id="customFrameName" placeholder="Например: Тройной фарт" maxlength="80"></label><label>Файл рамки<input id="customFrameFile" type="file" accept="image/*"></label><div class="row modal-actions"><button class="ok" onclick="uploadCustomFrame()">Загрузить рамку</button><button class="secondary" onclick="closeModal()">Отмена</button></div><div id="customFrameOut"></div></div>`);
}
async function uploadCustomFrame() {
  try {
    const name = val('customFrameName') || 'Кастомная рамка';
    const out = await apiUploadForm(`/api/campaigns/${state.currentCampaignId}/custom-frames`, 'customFrameFile', {name, description: '', rarity: 'unique'});
...
async function createAchievement() {
  try {
    ...
    await api(`/api/campaigns/${state.currentCampaignId}/achievements`, {method:'POST', body:{
      icon, icon_thumb, title, description: val('achDescription'), tag: val('achTag'), cosmetic_reward_id: val('achReward') || null, cosmetic_effect_reward_id: val('achEffectReward') || null, tag_reward_id: val('achTagReward') || null, currency_reward: num('achCurrencyReward', 0)
    }});
    state.campaignState = await api(`/api/campaigns/${state.currentCampaignId}/state`);
