...
TAG_LIBRARY = [
    ...
    {"id":"tag_hero","name":"Герой","description":"Простой героический тэг.","rarity":"common","category":"base","css_class":"tag-hero","emoji":"🛡️","sort_order":10},
    {"id":"tag_survivor","name":"Выживший","description":"Для тех, кто держится на последнем HP.","rarity":"common","category":"base","css_class":"tag-survivor","emoji":"💚","sort_order":20},
    {"id":"tag_scout","name":"Следопыт","description":"Тихий, внимательный и всегда впереди.","rarity":"common","category":"base","css_class":"tag-scout","emoji":"🍃","sort_order":30},
    {"id":"tag_netrunner","name":"Нетраннер","description":"Киберпространство знает твоё имя.","rarity":"rare","category":"shop","css_class":"tag-netrunner","emoji":"💻","sort_order":110},
    {"id":"tag_solo","name":"Соло","description":"Решает проблемы быстро и громко.","rarity":"rare","category":"shop","css_class":"tag-solo","emoji":"🔫","sort_order":120},
    {"id":"tag_lucky","name":"Любимец судьбы","description":"Кости подозрительно часто на твоей стороне.","rarity":"epic","category":"shop","css_class":"tag-lucky","emoji":"🍀","sort_order":210},
    {"id":"tag_stage_legend","name":"Легенда сцены","description":"Зал помнит этот выход.","rarity":"epic","category":"shop","css_class":"tag-stage","emoji":"🎤","sort_order":220},
    {"id":"tag_liquidator","name":"Ликвидатор","description":"Цель исчезает раньше, чем успевает понять угрозу.","rarity":"legendary","category":"shop","css_class":"tag-liquidator","emoji":"🎯","sort_order":310},
    {"id":"tag_war_spark","name":"Искра войны","description":"Один поступок изменил город.","rarity":"unique","category":"unique","css_class":"tag-war-spark","emoji":"💥","sort_order":410},
    {"id":"tag_reality_bug","name":"Ошибка реальности","description":"Когда хаос сработал лучше любого плана.","rarity":"unique","category":"unique","css_class":"tag-reality-bug","emoji":"🌀","sort_order":420},
]
...
def default_price_for_rarity(rarity: str | None) -> int | None:
    return RARITY_PRICES.get(str(rarity or "common"), 25)
...
class Database:
    ...
    def create_campaign(self, name: str, master_tg_id: int, *, injuries_enabled: bool = True, armor_enabled: bool = False) -> dict[str, Any]:
        code = self._new_invite_code()
        cur = self.conn.execute(
            """
            INSERT INTO campaigns(name, master_tg_id, rule_type, injuries_enabled, armor_enabled, invite_code, created_at)
            VALUES (?, ?, 'fantasy', ?, ?, ?, ?)
            """,
            (name.strip(), master_tg_id, int(injuries_enabled), int(armor_enabled), code, utc_now()),
        )
        self.conn.commit()
    ...
    def create_character(self, campaign_id: int, name: str, hp: int, ac: int, armor: int = 0, color: str = '#72a7ff') -> dict[str, Any]:
        ...
        cur = self.conn.execute(
            ...
            INSERT INTO characters(campaign_id, name, ac, max_hp_base, max_hp_penalty, current_hp, pain,
                                   armor_max_base, armor_max_penalty, armor_current, color, avatar_path, custom_frame, custom_effect, custom_tag, created_at)
            VALUES (?, ?, ?, ?, 0, ?, 0, ?, 0, ?, ?, '', '', '', '', ?)
            """,
    ...
    def update_character_fields(self, character_id: int, **fields: Any) -> dict[str, Any]:
        allowed = {
            ...
            "armor_max_base", "armor_max_penalty", "armor_current", "color", "avatar_path", "custom_frame", "custom_effect", "custom_tag", "statuses_json", "notes"
        }
    ...
    def seed_cosmetics(self) -> None:
        for item in COSMETIC_LIBRARY:
            self.conn.execute(
                """
                INSERT INTO cosmetics(id, name, description, rarity, category, css_class, asset_path, thumb_path, emoji, sort_order)
                VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
        ...
        for row in self._many("SELECT id, rarity FROM cosmetics"):
            price = default_price_for_rarity(row.get("rarity"))
    ...
    def seed_cosmetic_effects(self) -> None:
        ...
        for row in self._many("SELECT id, rarity FROM cosmetic_effects"):
            price = default_price_for_rarity(row.get("rarity"))
            purchasable = 0 if price is None else 1
    ...
    def list_cosmetic_effects(self) -> list[dict[str, Any]]:
        return self._many("SELECT * FROM cosmetic_effects ORDER BY sort_order, name")
    ...
    def create_custom_cosmetic_frame(self, *, frame_id: str, name: str, description: str = '', rarity: str = 'unique', asset_path: str = '', thumb_path: str = '', css_class: str = '', emoji: str = '🖼️') -> dict[str, Any]:
        ...
        self.conn.execute(
            ...
            ON CONFLICT(id) DO UPDATE SET
                ...
                sort_order=excluded.sort_order
            """,
            (frame_id, name.strip() or "Кастомная рамка", description.strip(), rarity.strip() or "legendary", css_class.strip(), asset_path.strip(), thumb_path.strip(), emoji.strip() or "🖼️"),
        )
        self.conn.commit()
    ...
    def list_achievements(self, campaign_id: int) -> list[dict[str, Any]]:
        rows = self._many("SELECT * FROM achievements WHERE campaign_id=? ORDER BY id DESC", (int(campaign_id),))
    ...
    def open_achievement_grant(self, grant_id: int, telegram_user_id: int) -> dict[str, Any]:
        ...
        if ach:
            ...
            if currency_reward > 0:
                ...
                if opened_at:
                    self.add_currency(int(telegram_user_id), currency_reward, reason=f"Достижение: {ach.get('title')}", source='achievement', source_id=int(grant["id"]))
    ...
    def purchasable_info(self, item: dict[str, Any]) -> dict[str, Any]:
        ...
        if item.get("price") is None:
            price = default_price_for_rarity(str(item.get("rarity") or "common"))
            item["price"] = price
    ...
    def purchase_cosmetic_item(self, telegram_user_id: int, item_type: str, item_id: str) -> dict[str, Any]:
        ...
        price = int(item.get("price") or default_price_for_rarity(rarity) or 0)
