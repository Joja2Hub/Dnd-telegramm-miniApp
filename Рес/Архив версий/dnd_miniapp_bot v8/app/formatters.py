...
def hp_line(char: dict[str, Any]) -> str:
    temp = int(char.get('temp_hp', 0) or 0)
    extra = f" + {temp} врем." if temp > 0 else ""
    return f"HP {char['current_hp']}/{char['current_max_hp']}{extra} (база {char['max_hp_base']}, штраф {char['max_hp_penalty']})"
...
def damage_result_master(res: rules.DamageResult, title: str = "Урон применён") -> str:
    ...
    if res.temp_hp_before or res.temp_hp_damage:
        lines.append(f"Временные HP: {res.temp_hp_before} → {res.temp_hp_after}, поглощено {res.temp_hp_damage}")
    ...
    if res.injuries_enabled:
        ...
        elif res.damage > 0 and res.temp_hp_damage and res.hp_damage == 0:
            lines.append("Урон поглощён временными HP.")
