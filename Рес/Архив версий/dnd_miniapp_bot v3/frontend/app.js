...
function desiredCanvasEffectState(el) {
  ...
  try {
    return !!(el.classList?.contains('effect-force-active') || wrapper?.classList?.contains('effect-force-active') || wrapper?.matches?.(':hover') || el?.matches?.(':hover'));
  } catch(_) {
    return !!(el.classList?.contains('effect-force-active') || wrapper?.classList?.contains('effect-force-active'));
  }
}
...
function sizeCanvasEffectHost(rec) {
  ...
  const base = Math.max(box.width || 0, box.height || 0, 48);
  const size = Math.max(176, Math.round(base * 2.45));
...
function bindEffectActivation() {
  document.querySelectorAll('.avatar.cosmetic-effect, .avatar-shell, .effect-choice-preview, .frame-demo.effect-demo').forEach(el => {
    ...
    el.addEventListener('click', ev => {
      // Не мешаем кнопкам покупки/выбора: активатор только показывает анимацию.
...
function renderUniqueCustomizationBlock(ch) {
  ...
  return `<div class="customization-sections compact-customization">
    ...
    <details class="custom-section tags custom-collapse" ${sectionAttrs('player-custom-tags')}>
      <summary><div class="unique-custom-head"><span>🏷️</span><div><b>Тэг</b><small>Комбинируй форму и текст: форма покупается отдельно, текст может быть куплен или получен за достижение.</small></div></div></summary>
      <div class="custom-collapse-body tag-composer">
        ${tagPreview}
        <div class="tag-style-title">Форма тэга</div>
        ${shapeHtml}
        <div class="tag-style-title">Текст тэга</div>
        ${textHtml}
      </div>
    </details>
  </div>`;
...
function renderShopBlock(ch) {
  return `<div class="card stack shop-standalone-card">
    ...
    <div class="shop-tabs">
      ...
      <details class="custom-section nested-shop" ${sectionAttrs('shop-tag-shapes')}><summary><b>🏷️ Формы тэгов</b></summary>${shopItemsHtml('tag', tagShapes().filter(x=>x.category !== 'base'))}</details>
      <details class="custom-section nested-shop" ${sectionAttrs('shop-tag-texts')}><summary><b>🔤 Тексты тэгов</b></summary>${shopItemsHtml('tag', tagTexts().filter(x=>x.rarity !== 'unique'))}</details>
...
function showLockedEffect(id) {
  ...
  showModal(`<div class="modal-card compact unique-modal locked-custom-modal"><button class="modal-close" onclick="closeModal()">×</button>
    ...
    <div class="locked-preview-card rarity-${esc(e?.rarity || 'legendary')} effect-card ${esc(cls)}"><div class="effect-choice-preview"><span class="frame-demo plain">○</span><span class="effect-preview-layer ${esc(cls)}" data-effect-id="${esc(id)}"></span></div><b>${esc(e?.name || id)}</b><small>${effectMetaLabel(e)}</small></div>
