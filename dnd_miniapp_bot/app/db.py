...
class Database:
    ...
    def init_schema(self) -> None:
        self.conn.executescript(
            ...
            CREATE TABLE IF NOT EXISTS campaigns (
                ...
                netrunner_character_id INTEGER,
    ...
    def _migrate(self) -> None:
        ...
        Важно: на живом сервере уже могут быть персонажи, искры, покупки,
        связи игроков и старые записи боёв. Поэтому миграции здесь только
        добавляют недостающие поля и никогда не пересоздают пользовательские
        таблицы с данными.
        """
        ...
        def backup_once() -> None:
            ...
            if backup_done or not self.path or self.path == ":memory:" or not os.path.exists(self.path):
                return
            try:
                backup_dir = os.path.join(os.path.dirname(self.path) or ".", "backups")
                os.makedirs(backup_dir, exist_ok=True)
                stamp = datetime.now(timezone.utc).strftime("%Y%m%d_%H%M%S")
                name = os.path.basename(self.path) or "database.sqlite3"
                target = os.path.join(backup_dir, f"{name}.pre_v67_{stamp}.bak")
                with sqlite3.connect(target) as dst:
                    self.conn.backup(dst)
                backup_done = True
            except Exception:
                # Миграция не должна падать только из-за невозможности создать backup.
                # Сами ALTER TABLE ниже остаются безопасными и добавляют только колонки.
                backup_done = True

        def add_col(table: str, existing: set[str], name: str, definition: str) -> None:
        ...
        add_col("campaigns", campaign_cols, "rule_type", "TEXT NOT NULL DEFAULT 'fantasy'")
        add_col("campaigns", campaign_cols, "injuries_enabled", "INTEGER NOT NULL DEFAULT 1")
        add_col("campaigns", campaign_cols, "armor_enabled", "INTEGER NOT NULL DEFAULT 0")
        add_col("campaigns", campaign_cols, "weapons_enabled", "INTEGER NOT NULL DEFAULT 0")
        add_col("campaigns", campaign_cols, "emoji", "TEXT NOT NULL DEFAULT '🎲'")
        add_col("campaigns", campaign_cols, "netrunner_character_id", "INTEGER")

        char_cols = cols("characters")
        add_col("characters", char_cols, "armor_max_base", "INTEGER NOT NULL DEFAULT 0")
        add_col("characters", char_cols, "armor_max_penalty", "INTEGER NOT NULL DEFAULT 0")
        add_col("characters", char_cols, "armor_current", "INTEGER NOT NULL DEFAULT 0")
        add_col("characters", char_cols, "color", "TEXT NOT NULL DEFAULT '#72a7ff'")
        add_col("characters", char_cols, "avatar_path", "TEXT NOT NULL DEFAULT ''")
        add_col("characters", char_cols, "avatar_thumb_path", "TEXT NOT NULL DEFAULT ''")
        add_col("characters", char_cols, "custom_frame", "TEXT NOT NULL DEFAULT ''")
        add_col("characters", char_cols, "custom_effect", "TEXT NOT NULL DEFAULT ''")
        add_col("characters", char_cols, "custom_tag", "TEXT NOT NULL DEFAULT ''")
        ...
        # до поля psych_effect. Без этих колонок любой урон, который создаёт
        # травму, превращался в HTTP 500.
        if has_table("injuries"):
            injury_cols = cols("injuries")
            add_col("injuries", injury_cols, "stabilized", "INTEGER NOT NULL DEFAULT 0")
            add_col("injuries", injury_cols, "healed", "INTEGER NOT NULL DEFAULT 0")
            add_col("injuries", injury_cols, "max_hp_loss", "INTEGER NOT NULL DEFAULT 0")
            add_col("injuries", injury_cols, "max_hp_restored", "INTEGER NOT NULL DEFAULT 0")
            add_col("injuries", injury_cols, "pain_added", "INTEGER NOT NULL DEFAULT 0")
            add_col("injuries", injury_cols, "combat", "TEXT NOT NULL DEFAULT ''")
            add_col("injuries", injury_cols, "exploration", "TEXT NOT NULL DEFAULT ''")
            add_col("injuries", injury_cols, "social", "TEXT NOT NULL DEFAULT ''")
            add_col("injuries", injury_cols, "notes", "TEXT NOT NULL DEFAULT ''")
            add_col("injuries", injury_cols, "heal_rule", "TEXT NOT NULL DEFAULT ''")
            add_col("injuries", injury_cols, "psych_effect", "TEXT")
            add_col("injuries", injury_cols, "created_at", "TEXT NOT NULL DEFAULT ''")
            self.conn.execute("UPDATE injuries SET created_at=? WHERE created_at='' OR created_at IS NULL", (utc_now(),))
        ...
        self.conn.executescript("""
            CREATE TABLE IF NOT EXISTS cyber_networks (
                id INTEGER PRIMARY KEY AUTOINCREMENT,
                campaign_id INTEGER NOT NULL REFERENCES campaigns(id) ON DELETE CASCADE,
                name TEXT NOT NULL,
                depth INTEGER NOT NULL DEFAULT 4,
                regen_every INTEGER NOT NULL DEFAULT 3,
                nodes_json TEXT NOT NULL DEFAULT '[]',
                created_at TEXT NOT NULL,
                updated_at TEXT NOT NULL
            );
            CREATE TABLE IF NOT EXISTS cyber_network_sessions (
                id INTEGER PRIMARY KEY AUTOINCREMENT,
                campaign_id INTEGER NOT NULL UNIQUE REFERENCES campaigns(id) ON DELETE CASCADE,
                network_id INTEGER NOT NULL REFERENCES cyber_networks(id) ON DELETE CASCADE,
                runner_character_id INTEGER NOT NULL REFERENCES characters(id) ON DELETE CASCADE,
                state_json TEXT NOT NULL DEFAULT '{}',
                status TEXT NOT NULL DEFAULT 'active',
                created_at TEXT NOT NULL,
                updated_at TEXT NOT NULL
            );
            CREATE TABLE IF NOT EXISTS session_summary_dispatches (
                id INTEGER PRIMARY KEY AUTOINCREMENT,
                campaign_id INTEGER NOT NULL REFERENCES campaigns(id) ON DELETE CASCADE,
                character_id INTEGER NOT NULL REFERENCES characters(id) ON DELETE CASCADE,
                session_date TEXT NOT NULL,
                sent_at TEXT NOT NULL,
                UNIQUE(campaign_id, character_id, session_date)
            );
            CREATE TABLE IF NOT EXISTS upload_dedup_records (
            ...
            CREATE TABLE IF NOT EXISTS cyber_inventory_slots (
                character_id INTEGER NOT NULL REFERENCES characters(id) ON DELETE CASCADE,
                mode TEXT NOT NULL,
                slot_id TEXT NOT NULL,
                item_id INTEGER REFERENCES inventory_items(id) ON DELETE SET NULL,
                updated_at TEXT NOT NULL,
                PRIMARY KEY(character_id, mode, slot_id)
            );
            CREATE INDEX IF NOT EXISTS idx_cyber_inventory_slots_item ON cyber_inventory_slots(item_id);
        """)
        ...
        if "sort_order" not in inv_cols:
            self.conn.execute("ALTER TABLE inventory_items ADD COLUMN sort_order INTEGER NOT NULL DEFAULT 0")
            # v69: сохраняем привычный порядок старых инвентарей: новые предметы были сверху по id DESC.
            self.conn.execute("UPDATE inventory_items SET sort_order=id WHERE COALESCE(sort_order, 0)=0")
    ...
    def get_recent_upload_result(self, dedup_key: str, *, max_age_seconds: int = 600) -> dict[str, Any] | None:
        threshold = (datetime.now(timezone.utc) - timedelta(seconds=max(1, int(max_age_seconds)))).isoformat(timespec="seconds")
        row = self._one(
            "SELECT response_json FROM upload_dedup_records WHERE dedup_key=? AND created_at>=?",
            (str(dedup_key), threshold),
        )
        if not row:
            return None
        try:
            value = json.loads(row.get("response_json") or "{}")
            return value if isinstance(value, dict) else None
        except Exception:
            return None

    def save_upload_result(self, dedup_key: str, response: dict[str, Any]) -> None:
        self.conn.execute(
            """
            INSERT INTO upload_dedup_records(dedup_key, response_json, created_at)
            VALUES (?, ?, ?)
            ON CONFLICT(dedup_key) DO UPDATE SET response_json=excluded.response_json, created_at=excluded.created_at
            """,
            (str(dedup_key), json.dumps(response, ensure_ascii=False), utc_now()),
        )
        self.conn.execute(
            "DELETE FROM upload_dedup_records WHERE created_at<?",
    ...
    def list_session_campaigns(
        ...
        *,
        ...
        session_kinds = (
            "damage", "miss", "combat", "combat_damage", "manual",
            "full_heal", "armor_repair", "status", "request", "injury",
            "inventory", "cyber", "undo",
        )
        marks = ",".join("?" for _ in session_kinds)
        sql = f"""
            SELECT c.*, MIN(e.created_at) AS session_started_at, MAX(e.created_at) AS session_last_activity_at
            FROM campaigns c
            JOIN log_events e ON e.campaign_id=c.id
            WHERE e.created_at>=? AND e.created_at<? AND e.kind IN ({marks})
        """
        params: list[Any] = [str(start_utc), str(end_utc), *session_kinds]
        if master_tg_id:
            sql += " AND c.master_tg_id=?"
            params.append(int(master_tg_id))
        sql += " GROUP BY c.id ORDER BY session_last_activity_at"
        return [self._hydrate_campaign(row) for row in self._many(sql, tuple(params))]

    def session_summary_was_sent(self, campaign_id: int, character_id: int, session_date: str) -> bool:
        return bool(self._one(
            "SELECT id FROM session_summary_dispatches WHERE campaign_id=? AND character_id=? AND session_date=?",
            (int(campaign_id), int(character_id), str(session_date)),
        ))

    def mark_session_summary_sent(self, campaign_id: int, character_id: int, session_date: str) -> None:
        self.conn.execute(
            """
            INSERT INTO session_summary_dispatches(campaign_id, character_id, session_date, sent_at)
            VALUES (?, ?, ?, ?)
            ON CONFLICT(campaign_id, character_id, session_date) DO UPDATE SET sent_at=excluded.sent_at
            """,
            (int(campaign_id), int(character_id), str(session_date), utc_now()),
        )
        self.conn.commit()
    ...
    def create_campaign(self, name: str, master_tg_id: int, *, rule_type: str = "fantasy", injuries_enabled: bool = True, armor_enabled: bool = False, weapons_enabled: bool = False) -> dict[str, Any]:
        code = self._new_invite_code()
        rule_type = "cyberpunk" if str(rule_type) == "cyberpunk" else "fantasy"
        cur = self.conn.execute(
            """
            INSERT INTO campaigns(name, master_tg_id, rule_type, injuries_enabled, armor_enabled, weapons_enabled, invite_code, created_at)
            VALUES (?, ?, ?, ?, ?, ?, ?, ?)
            """,
            (name.strip(), master_tg_id, rule_type, int(injuries_enabled), int(armor_enabled), int(weapons_enabled), code, utc_now()),
        )
        self.conn.commit()
    ...
    def _hydrate_campaign(self, row: dict[str, Any]) -> dict[str, Any]:
        ...
        row["rule_type"] = "cyberpunk" if row.get("rule_type") == "cyberpunk" else "fantasy"
        row["netrunner_character_id"] = row.get("netrunner_character_id")
        row["emoji"] = str(row.get("emoji", "🎲") or "🎲")
        return row

    def update_campaign_settings(self, campaign_id: int, *, name: str | None = None, emoji: str | None = None, rule_type: str | None = None, netrunner_character_id: object = ...) -> dict[str, Any]:
        updates: dict[str, Any] = {}
        if name is not None:
            clean = str(name).strip()
            if not clean:
                raise ValueError("Название кампании не может быть пустым")
            updates["name"] = clean[:80]
        if emoji is not None:
            updates["emoji"] = str(emoji).strip()[:8] or "🎲"
        if rule_type is not None:
            updates["rule_type"] = "cyberpunk" if str(rule_type) == "cyberpunk" else "fantasy"
        if netrunner_character_id is not ...:
            updates["netrunner_character_id"] = int(netrunner_character_id) if netrunner_character_id else None
        if updates:
            cols = ", ".join(f"{k}=?" for k in updates)
            self.conn.execute(f"UPDATE campaigns SET {cols} WHERE id=?", (*updates.values(), int(campaign_id)))
            self.conn.commit()
        c = self.get_campaign(int(campaign_id))
        if not c:
            raise ValueError("Кампания не найдена")
        return c

    # ---------- cyberpunk netrunner ----------
    def _hydrate_cyber_network(self, row: dict[str, Any] | None) -> dict[str, Any] | None:
        if not row:
            return None
        data = dict(row)
        try:
    ...
    def list_cyber_networks(self, campaign_id: int) -> list[dict[str, Any]]:
        return [self._hydrate_cyber_network(row) for row in self._many(
            "SELECT * FROM cyber_networks WHERE campaign_id=? ORDER BY id DESC", (int(campaign_id),)
        ) if row]  # type: ignore[list-item]

    def get_cyber_network(self, network_id: int, campaign_id: int | None = None) -> dict[str, Any] | None:
        sql = "SELECT * FROM cyber_networks WHERE id=?"
        params: tuple[Any, ...] = (int(network_id),)
        if campaign_id is not None:
            sql += " AND campaign_id=?"
            params += (int(campaign_id),)
        return self._hydrate_cyber_network(self._one(sql, params))

    def create_cyber_network(self, campaign_id: int, name: str, depth: int, regen_every: int, nodes: list[dict[str, Any]]) -> dict[str, Any]:
        clean_nodes = [dict(node) for node in nodes if isinstance(node, dict)][:100]
        for node in clean_nodes:
            node["id"] = str(node.get("id") or "")[:80]
            node["parentId"] = str(node.get("parentId") or "root")[:80]
            node["level"] = max(1, min(9, int(node.get("level") or 1)))
            node["children"] = []
        ids = {node["id"] for node in clean_nodes}
        for node in clean_nodes:
            parent = node.get("parentId")
            if parent != "root" and parent in ids:
                owner = next((item for item in clean_nodes if item["id"] == parent), None)
                if owner is not None:
                    owner["children"].append(node["id"])
        now = utc_now()
        cur = self.conn.execute(
            "INSERT INTO cyber_networks(campaign_id, name, depth, regen_every, nodes_json, created_at, updated_at) VALUES (?, ?, ?, ?, ?, ?, ?)",
            (int(campaign_id), str(name).strip()[:80], max(1, min(9, int(depth))), max(0, min(20, int(regen_every))), json.dumps(clean_nodes, ensure_ascii=False), now, now),
        )
        self.conn.commit()
        return self.get_cyber_network(int(cur.lastrowid))  # type: ignore[return-value]

    def get_cyber_session(self, campaign_id: int) -> dict[str, Any] | None:
        row = self._one(
            "SELECT s.*, n.name AS network_name, n.depth AS network_depth, n.regen_every, n.nodes_json FROM cyber_network_sessions s JOIN cyber_networks n ON n.id=s.network_id WHERE s.campaign_id=?",
            (int(campaign_id),),
        )
        if not row:
            return None
        try:
            state = json.loads(row.pop("state_json") or "{}")
        except Exception:
            state = {}
        try:
            nodes = json.loads(row.pop("nodes_json") or "[]")
        except Exception:
            nodes = []
        row["state"] = state
        row["network"] = {"id": row["network_id"], "name": row.pop("network_name"), "depth": row.pop("network_depth"), "regen_every": row.pop("regen_every"), "nodes": nodes}
        return row

    def start_cyber_session(self, campaign_id: int, network_id: int, runner_character_id: int) -> dict[str, Any]:
        network = self.get_cyber_network(network_id, campaign_id)
        runner = self.get_character(runner_character_id)
        if not network or not runner or int(runner["campaign_id"]) != int(campaign_id):
            raise ValueError("Сеть или нетраннер не найдены")
        nodes = json.loads(json.dumps(network.get("nodes") or [], ensure_ascii=False))
        for node in nodes:
            guardian = node.get("guardian") if isinstance(node, dict) else None
            if isinstance(guardian, dict):
                guardian["maxHp"] = max(1, int(guardian.get("maxHp") or guardian.get("hp") or 10))
                guardian["hp"] = guardian["maxHp"]
        state = {
            "current_node_id": "root", "revealed_details": ["root"], "cleared": ["root"],
            "actions_max": 8, "actions_left": 8, "regen_countdown": int(network.get("regen_every") or 0),
            "runner": {"hp": int(runner.get("current_hp") or 0), "max_hp": int(runner.get("current_max_hp") or runner.get("max_hp_base") or 1), "net_hp": 18, "max_net_hp": 18, "ac": int(runner.get("ac") or 10)},
            "nodes": nodes, "hack_request": None, "log": ["Сеть запущена."],
        }
        now = utc_now()
        self.conn.execute(
            "INSERT INTO cyber_network_sessions(campaign_id, network_id, runner_character_id, state_json, status, created_at, updated_at) VALUES (?, ?, ?, ?, 'active', ?, ?) ON CONFLICT(campaign_id) DO UPDATE SET network_id=excluded.network_id, runner_character_id=excluded.runner_character_id, state_json=excluded.state_json, status='active', updated_at=excluded.updated_at",
            (int(campaign_id), int(network_id), int(runner_character_id), json.dumps(state, ensure_ascii=False), now, now),
        )
        self.conn.commit()
        return self.get_cyber_session(campaign_id)  # type: ignore[return-value]

    def save_cyber_session_state(self, campaign_id: int, state: dict[str, Any]) -> dict[str, Any]:
        self.conn.execute("UPDATE cyber_network_sessions SET state_json=?, updated_at=? WHERE campaign_id=?", (json.dumps(state, ensure_ascii=False), utc_now(), int(campaign_id)))
        self.conn.commit()
        return self.get_cyber_session(campaign_id)  # type: ignore[return-value]
    ...
    def create_character(self, campaign_id: int, name: str, hp: int, ac: int, armor: int = 0, color: str = '#72a7ff') -> dict[str, Any]:
        ...
        cur = self.conn.execute(
            ...
            INSERT INTO characters(campaign_id, name, ac, max_hp_base, max_hp_penalty, current_hp, pain,
                                   armor_max_base, armor_max_penalty, armor_current, color, avatar_path, custom_frame, custom_effect, custom_tag, custom_tag_text, custom_tag_style, temp_hp, created_at)
            VALUES (?, ?, ?, ?, 0, ?, 0, ?, 0, ?, ?, '', '', '', '', '', 'tag_none', 0, ?)
            """,
    ...
    def update_character_fields(self, character_id: int, **fields: Any) -> dict[str, Any]:
        ...
        if "temp_hp" in data:
            data["temp_hp"] = max(0, int(data.get("temp_hp") or 0))
        if not data:
    ...
    def _hydrate_character(self, row: dict[str, Any]) -> dict[str, Any]:
        ...
        row["temp_hp"] = max(0, int(row.get("temp_hp", 0) or 0))
        ...
        row["campaign_emoji"] = campaign.get("emoji", "🎲") if campaign else "🎲"
    ...
    def restore_character_snapshot(self, snapshot: dict[str, Any]) -> dict[str, Any] | None:
        ...
        fields = {
            ...
            "temp_hp": snapshot.get("temp_hp", 0),
            ...
            "custom_tag_text": snapshot.get("custom_tag_text", ""),
            "custom_tag_style": snapshot.get("custom_tag_style", "tag_none"),
        ...
        if self.get_character(character_id) is None:
            self.conn.execute(
                """
                INSERT INTO characters(id, campaign_id, name, telegram_user_id, ac, max_hp_base, max_hp_penalty, current_hp, temp_hp, pain,
                                       armor_max_base, armor_max_penalty, armor_current, color, avatar_path, custom_frame, custom_effect, custom_tag, custom_tag_text, custom_tag_style, statuses_json, notes, created_at)
                VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
                """,
                (
                    ...
                    snapshot.get("current_hp", 1), snapshot.get("temp_hp", 0), snapshot.get("pain", 0), snapshot.get("armor_max_base", 0),
                    snapshot.get("armor_max_penalty", 0), snapshot.get("armor_current", 0), snapshot.get("color", "#72a7ff"), snapshot.get("avatar_path", ""), snapshot.get("custom_frame", ""), snapshot.get("custom_effect", ""), snapshot.get("custom_tag", ""), snapshot.get("custom_tag_text", ""), snapshot.get("custom_tag_style", "tag_none"), statuses_json, snapshot.get("notes", ""),
        ...
        models = [
            {
                ...
                "asset_path": "/assets/inventory-models/common/arcane_figure.png",
                "thumb_path": "/assets/inventory-models/common/arcane_figure.png",
                "rarity": "common",
                "category": "base",
                "width_px": 226,
                "height_px": 390,
            ...
            },
            {
                ...
                "asset_path": "/assets/inventory-models/common/sword_warrior.png",
                "thumb_path": "/assets/inventory-models/common/sword_warrior.png",
                "rarity": "common",
                "category": "base",
                "width_px": 226,
                "height_px": 390,
            ...
            },
            {
                ...
                "asset_path": "/assets/inventory-models/common/hooded_rogue.png",
                "thumb_path": "/assets/inventory-models/common/hooded_rogue.png",
                "rarity": "common",
                "category": "base",
                "width_px": 226,
                "height_px": 390,
            ...
            },
            {
                "id": "cyberpunk_female",
                "name": "Женская модель",
                "description": "Базовая киберпанк-модель инвентаря.",
                "asset_path": "/assets/inventory-models/cyberpunk/female.png",
                "thumb_path": "/assets/inventory-models/cyberpunk/female_thumb.png",
                "rarity": "common",
                "category": "cyberpunk_base",
                "width_px": 140,
                "height_px": 340,
                "offset_y": 4,
                "glow_css": "drop-shadow(0 0 24px rgba(255,87,128,.38)) drop-shadow(0 0 18px rgba(48,246,255,.16))",
                "sort_order": 40,
            },
            {
                "id": "cyberpunk_male",
                "name": "Мужская модель",
                "description": "Базовая мужская киберпанк-модель инвентаря.",
                "asset_path": "/assets/inventory-models/cyberpunk/male.png",
                "thumb_path": "/assets/inventory-models/cyberpunk/male_thumb.png",
                "rarity": "common",
                "category": "cyberpunk_base",
                "width_px": 150,
                "height_px": 340,
                "offset_y": 6,
                "glow_css": "drop-shadow(0 0 24px rgba(255,87,128,.36)) drop-shadow(0 0 20px rgba(48,246,255,.20))",
                "sort_order": 50,
            },
            {
                "id": "cyberpunk_vladimir",
                "name": "Владимир",
                ...
                "height_px": 346,
                "offset_y": 8,
                "glow_css": "drop-shadow(0 0 24px rgba(255,74,92,.28)) drop-shadow(0 0 18px rgba(48,246,255,.14))",
                "sort_order": 60,
            },
            {
                "id": "cyberpunk_vladimir_suit",
                "name": "Владимир: костюм",
                ...
                "height_px": 346,
                "offset_y": 8,
                "glow_css": "drop-shadow(0 0 22px rgba(60,190,255,.24)) drop-shadow(0 0 16px rgba(255,74,92,.16))",
                "sort_order": 61,
            },
            {
                "id": "cyberpunk_gerda",
                ...
                "height_px": 418,
                "offset_y": 8,
                "glow_css": "drop-shadow(0 0 22px rgba(48,246,255,.20)) drop-shadow(0 0 16px rgba(255,150,64,.16))",
                "sort_order": 70,
            },
            {
                "id": "cyberpunk_gerda_dress",
                "name": "Герда: образ",
                ...
                "height_px": 342,
                "offset_y": 8,
                "glow_css": "drop-shadow(0 0 24px rgba(84,210,255,.26)) drop-shadow(0 0 18px rgba(230,245,255,.18))",
                "sort_order": 71,
            },
            {
                "id": "cyberpunk_raven",
                ...
                "height_px": 342,
                "offset_y": 8,
                "glow_css": "drop-shadow(0 0 24px rgba(48,140,255,.28)) drop-shadow(0 0 18px rgba(180,80,255,.16))",
                "sort_order": 80,
            },
            {
                "id": "cyberpunk_raven_dress",
                "name": "Рейвн: образ",
                ...
                "height_px": 347,
                "offset_y": 4,
                "glow_css": "drop-shadow(0 0 24px rgba(48,140,255,.30)) drop-shadow(0 0 18px rgba(90,210,255,.18))",
                "sort_order": 81,
            },
            {
                "id": "cyberpunk_hex",
                ...
                "height_px": 340,
                "offset_y": 10,
                "glow_css": "drop-shadow(0 0 22px rgba(48,246,255,.24)) drop-shadow(0 0 16px rgba(255,74,92,.12))",
                "sort_order": 90,
            },
            {
                "id": "cyberpunk_hex_pistol_car",
                "name": "Хекс: машина",
                ...
                "height_px": 286,
                "offset_y": 26,
                "glow_css": "drop-shadow(0 0 24px rgba(48,246,255,.22)) drop-shadow(0 0 18px rgba(178,80,255,.18))",
                "sort_order": 91,
            },
            {
                "id": "cyberpunk_hex_lansko",
                "name": "Хекс: Lansko",
                ...
                "height_px": 362,
                "offset_y": 12,
                "glow_css": "drop-shadow(0 0 22px rgba(48,246,255,.24)) drop-shadow(0 0 16px rgba(255,74,92,.14))",
                "sort_order": 92,
            },
            {
                "id": "cyberpunk_eclipse",
                "name": "Эклипс",
                ...
                "height_px": 340,
                "offset_y": 8,
                "glow_css": "drop-shadow(0 0 24px rgba(255,52,127,.30)) drop-shadow(0 0 18px rgba(48,246,255,.14))",
                "sort_order": 100,
            },
            {
                "id": "cyberpunk_eclipse_look",
                "name": "Эклипс: образ",
                ...
                "height_px": 318,
                "offset_y": -4,
                "glow_css": "drop-shadow(0 0 24px rgba(255,52,127,.26)) drop-shadow(0 0 18px rgba(48,246,255,.16))",
                "sort_order": 101,
            },
            {
                "id": "mesivo_delfin",
                "campaign_name": "Месиво",
                "name": "Дельфин",
                "description": "Водная чародейка со светящейся сферой.",
                "asset_path": "/assets/inventory-models/mesivo/delfin.png",
                "thumb_path": "/assets/inventory-models/mesivo/delfin_thumb.png",
                "rarity": "unique",
                "category": "campaign",
                "width_px": 230,
                "height_px": 380,
                "offset_y": 8,
                "glow_css": "drop-shadow(0 12px 24px rgba(6,20,34,.36)) drop-shadow(0 0 18px rgba(91,201,232,.26))",
                "sort_order": 110,
            },
            {
                "id": "mesivo_kavushka",
                "campaign_name": "Месиво",
                "name": "Кавушка",
                "description": "Маленький жабий мудрец в монашеских одеждах.",
                "asset_path": "/assets/inventory-models/mesivo/kavushka.png",
                "thumb_path": "/assets/inventory-models/mesivo/kavushka_thumb.png",
                "rarity": "unique",
                "category": "campaign",
                "width_px": 214,
                "height_px": 338,
                "offset_y": 22,
                "glow_css": "drop-shadow(0 12px 22px rgba(34,18,6,.34)) drop-shadow(0 0 16px rgba(220,132,42,.20))",
                "sort_order": 120,
            },
            {
                "id": "mesivo_kayra",
                "campaign_name": "Месиво",
                "name": "Кайра",
                "description": "Лионинка-паладин с мечом и щитом.",
                "asset_path": "/assets/inventory-models/mesivo/kayra.png",
                "thumb_path": "/assets/inventory-models/mesivo/kayra_thumb.png",
                "rarity": "unique",
                "category": "campaign",
                "width_px": 210,
                "height_px": 370,
                "offset_y": 14,
                "glow_css": "drop-shadow(0 14px 26px rgba(18,14,10,.38)) drop-shadow(0 0 16px rgba(216,181,108,.18))",
                "sort_order": 130,
            },
            {
                "id": "mesivo_ksael",
                "campaign_name": "Месиво",
                "name": "Ксаэль",
                "description": "Юань-ти бард с ледяной рапирой и флейтой.",
                "asset_path": "/assets/inventory-models/mesivo/ksael.png",
                "thumb_path": "/assets/inventory-models/mesivo/ksael_thumb.png",
                "rarity": "unique",
                "category": "campaign",
                "width_px": 226,
                "height_px": 388,
                "offset_y": 8,
                "glow_css": "drop-shadow(0 12px 24px rgba(4,18,22,.36)) drop-shadow(0 0 18px rgba(99,211,232,.20))",
                "sort_order": 140,
            },
            {
                "id": "mesivo_kyuraku",
                "campaign_name": "Месиво",
                "name": "Кьюраку",
                "description": "Три-крин самурай с черной метеоритной катаной и мимиком-сумкой.",
                "asset_path": "/assets/inventory-models/mesivo/kyuraku.png",
                "thumb_path": "/assets/inventory-models/mesivo/kyuraku_thumb.png",
                "rarity": "unique",
                "category": "campaign",
                "width_px": 230,
                "height_px": 390,
                "offset_y": 8,
                "glow_css": "drop-shadow(0 14px 26px rgba(12,10,7,.38)) drop-shadow(0 0 16px rgba(204,142,70,.18))",
                "sort_order": 150,
            },
            {
                "id": "mesivo_mishan",
                "campaign_name": "Месиво",
                "name": "Мисхан",
                "description": "Драколюдка с черной чешуей, золотой броней и красными камнями.",
                "asset_path": "/assets/inventory-models/mesivo/mishan.png",
                "thumb_path": "/assets/inventory-models/mesivo/mishan_thumb.png",
                "rarity": "unique",
                "category": "campaign",
                "width_px": 220,
                "height_px": 390,
                "offset_y": 8,
                "glow_css": "drop-shadow(0 14px 26px rgba(18,10,6,.38)) drop-shadow(0 0 16px rgba(230,45,45,.18))",
                "sort_order": 160,
            },
            {
                "id": "mesivo_yan_dragon",
                "campaign_name": "Месиво",
                "name": "Ян: дракон",
                "description": "Небольшая драконья форма Яна.",
                "asset_path": "/assets/inventory-models/mesivo/yan_dragon.png",
                "thumb_path": "/assets/inventory-models/mesivo/yan_dragon_thumb.png",
                "rarity": "unique",
                "category": "campaign",
                "width_px": 278,
                "height_px": 389,
                "offset_y": 0,
                "glow_css": "drop-shadow(0 12px 22px rgba(42,18,6,.34)) drop-shadow(0 0 16px rgba(238,139,55,.20))",
                "sort_order": 170,
            },
            {
                "id": "mesivo_yan_human",
                "campaign_name": "Месиво",
                "name": "Ян: человек",
                "description": "Человеческая форма Яна в народной одежде с посохом.",
                "asset_path": "/assets/inventory-models/mesivo/yan_human.png",
                "thumb_path": "/assets/inventory-models/mesivo/yan_human_thumb.png",
                "rarity": "unique",
                "category": "campaign",
                "width_px": 222,
                "height_px": 390,
                "offset_y": 8,
                "glow_css": "drop-shadow(0 12px 24px rgba(36,20,10,.34)) drop-shadow(0 0 14px rgba(197,48,48,.16))",
                "sort_order": 180,
            },
            {
                "id": "oskolok_doctor",
                "campaign_name": "Осколок",
                "name": "Врач",
                "description": "Мрачный доктор-алхимик с флаконами, цилиндром и закрытым зонтом.",
                "asset_path": "/assets/inventory-models/oskolok/doctor.png",
                "thumb_path": "/assets/inventory-models/oskolok/doctor_thumb.png",
                "rarity": "unique",
                "category": "campaign",
                "width_px": 218,
                "height_px": 390,
                "offset_y": 8,
                "glow_css": "drop-shadow(0 14px 28px rgba(3,8,7,.46)) drop-shadow(0 0 16px rgba(122,170,75,.18))",
                "sort_order": 210,
            },
            {
                "id": "oskolok_olgood",
                "campaign_name": "Осколок",
                "name": "Олгуд",
                "description": "Мрачный рыцарь в побитой броне с щитом, мечом и фонарём.",
                "asset_path": "/assets/inventory-models/oskolok/olgood.png",
                "thumb_path": "/assets/inventory-models/oskolok/olgood_thumb.png",
                "rarity": "unique",
                "category": "campaign",
                "width_px": 232,
                "height_px": 390,
                "offset_y": 8,
                "glow_css": "drop-shadow(0 14px 28px rgba(5,5,4,.46)) drop-shadow(0 0 16px rgba(214,133,45,.18))",
                "sort_order": 220,
            },
            {
                "id": "oskolok_tianna",
                "campaign_name": "Осколок",
                "name": "Тианна",
                "description": "Тёмная фейская прорицательница с картами, лиловой магией и шутовским колпаком.",
                "asset_path": "/assets/inventory-models/oskolok/tianna.png",
                "thumb_path": "/assets/inventory-models/oskolok/tianna_thumb.png",
                "rarity": "unique",
                "category": "campaign",
                "width_px": 156,
                "height_px": 375,
                "offset_y": 18,
                "glow_css": "drop-shadow(0 14px 28px rgba(10,4,14,.44)) drop-shadow(0 0 18px rgba(188,92,224,.20))",
                "sort_order": 230,
            },
            {
                "id": "oskolok_vellori",
                "campaign_name": "Осколок",
                "name": "Вэллори",
                "description": "Тёмная фейская бардесса со звёздным костюмом, колпаком и лютней.",
                "asset_path": "/assets/inventory-models/oskolok/vellori.png",
                "thumb_path": "/assets/inventory-models/oskolok/vellori_thumb.png",
                "rarity": "unique",
                "category": "campaign",
                "width_px": 186,
                "height_px": 318,
                "offset_y": 2,
                "glow_css": "drop-shadow(0 14px 28px rgba(8,5,16,.44)) drop-shadow(0 0 18px rgba(128,104,232,.18))",
                "sort_order": 240,
            },
        ]
        for item in models:
            campaign_id = item.get("campaign_id")
            if campaign_id is None and item.get("campaign_name"):
                row = self._one("SELECT id FROM campaigns WHERE lower(name)=lower(?)", (str(item["campaign_name"]),))
            ...
            self.conn.execute(
                ...
                """,
                (
                    item["id"], campaign_id, item["name"], item["description"], item["asset_path"], item["thumb_path"],
    ...
    def grant_achievement(self, achievement_id: int, character_id: int, master_tg_id: int, master_comment: str = '') -> dict[str, Any]:
        ...
        cur = self.conn.execute(
            ...
            """,
            (int(achievement_id), int(tg_id), int(ch["campaign_id"]), int(character_id), int(master_tg_id), master_comment.strip(), utc_now()),
        )
    ...
    def revoke_achievement_from_user(self, achievement_id: int, telegram_user_id: int) -> bool:
        rows = self._many("SELECT id FROM achievement_grants WHERE achievement_id=? AND telegram_user_id=?", (int(achievement_id), int(telegram_user_id)))
        grant_ids = [int(r["id"]) for r in rows]
        if not grant_ids:
            return False
        placeholders = ",".join("?" for _ in grant_ids)
        self.conn.execute(f"DELETE FROM user_cosmetic_unlocks WHERE source='achievement' AND source_id IN ({placeholders})", tuple(grant_ids))
    ...
    def list_spark_players(self, limit_history: int = 10) -> list[dict[str, Any]]:
        rows = self._many("""
            SELECT telegram_user_id, MAX(id) AS last_character_id
            FROM characters
            WHERE telegram_user_id IS NOT NULL
            GROUP BY telegram_user_id
            ORDER BY last_character_id DESC
            """)
        out: list[dict[str, Any]] = []
        for r in rows:
            tg_id = int(r["telegram_user_id"])
            profile = self.get_user_profile(tg_id)
            chars = profile.get("characters") or []
            main_char = profile.get("main_character") or (chars[0] if chars else None)
            profile["display_name"] = (main_char or {}).get("name") or f"Игрок TG {tg_id}"
            profile["currency_transactions"] = self.list_currency_transactions(tg_id, limit_history)
            profile["achievement_grants"] = self.list_player_achievement_grants(tg_id)
            out.append(profile)
        return out
    ...
    def spark_management_state(self, master_tg_id: int, *, is_admin: bool = False) -> dict[str, Any]:
        ...
        return {
            ...
            "players": self.list_spark_players(8) if is_admin else [],
            "achievements": self.list_achievements() if is_admin else [],
        }
    ...
    def list_inventory(self, character_id: int) -> list[dict[str, Any]]:
        rows = self._many("SELECT * FROM inventory_items WHERE character_id=? ORDER BY sort_order DESC, id DESC", (int(character_id),))
        return [self._hydrate_inventory_item(r) for r in rows]

    def list_cyber_inventory_slots(self, character_id: int) -> list[dict[str, Any]]:
        rows = self._many(
            """
            SELECT s.mode, s.slot_id, s.item_id
            FROM cyber_inventory_slots s
            LEFT JOIN inventory_items i ON i.id=s.item_id
            WHERE s.character_id=? AND (s.item_id IS NULL OR i.id IS NOT NULL)
            ORDER BY s.mode, s.slot_id
            """,
            (int(character_id),),
        )
        return [
    ...
    def set_cyber_inventory_slot(self, character_id: int, mode: str, slot_id: str, item_id: int | None) -> list[dict[str, Any]]:
        ...
        if item_id:
            ...
            if not item or int(item.get("character_id") or 0) != int(character_id):
                raise ValueError("Предмет не найден в инвентаре персонажа")
            self.conn.execute(
                "DELETE FROM cyber_inventory_slots WHERE character_id=? AND item_id=?",
                (int(character_id), int(item_id)),
            )
            self.conn.execute(
                """
                INSERT INTO cyber_inventory_slots(character_id, mode, slot_id, item_id, updated_at)
                VALUES (?, ?, ?, ?, ?)
                ON CONFLICT(character_id, mode, slot_id) DO UPDATE SET item_id=excluded.item_id, updated_at=excluded.updated_at
                """,
                (int(character_id), mode, slot_id, int(item_id), now),
            )
        else:
            self.conn.execute(
                "DELETE FROM cyber_inventory_slots WHERE character_id=? AND mode=? AND slot_id=?",
                (int(character_id), mode, slot_id),
            )
        self.conn.commit()
        return self.list_cyber_inventory_slots(character_id)
    ...
    def create_inventory_item(self, character_id: int, *, name: str, description: str = '', emoji: str = '', quantity: int = 1, item_type: str = 'normal', weapon_type: str = '', reload_type: str = 'magazine', mag_capacity: int = 0, ammo_per_attack: int = 0, magazine_count: int = 0, fire_modes: list[dict[str, Any]] | None = None, magazines: list[dict[str, Any]] | None = None, shell_stocks: list[dict[str, Any]] | None = None, loaded_count: int = 0) -> dict[str, Any]:
        ...
        sort_order = int((self._one("SELECT COALESCE(MAX(sort_order), 0)+1 AS n FROM inventory_items WHERE character_id=?", (int(character_id),)) or {}).get('n') or 1)
        cur = self.conn.execute(
            """
            INSERT INTO inventory_items(character_id, item_type, name, description, emoji, quantity, weapon_type, reload_type, mag_capacity, ammo_per_attack, sort_order, created_at, updated_at)
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
            """,
            (int(character_id), item_type, name.strip()[:100], description.strip()[:1000], emoji.strip()[:8], max(1, int(quantity or 1)), weapon_type[:40], reload_type, max(0, int(mag_capacity or 0)), max(0, int(ammo_per_attack or 0)), sort_order, now, now),
        )
        ...
        if item_type == 'weapon':
            ...
            if reload_type == 'magazine':
                ...
                if not mags and int(magazine_count or 0) > 0:
                    default_capacity = max(1, int(mag_capacity or 1))
                    mags = [
                        {
                            "name": f"Магазин {i + 1}",
                            "ammo_max": default_capacity,
                            "ammo_current": 0,
                            "ammo_type": "",
                            "description": "",
                        }
                        for i in range(min(16, int(magazine_count or 0)))
                    ]
                active_id = None
                for i, m in enumerate(mags[:16]):
                    ...
                    mcur = self.conn.execute("""
                        ...
                        VALUES (?, ?, ?, ?, ?, ?, ?)
                    """, (item_id, str(m.get('name') or f"Магазин {i+1}")[:80], cur_ammo, max_ammo, str(m.get('ammo_type') if m.get('ammo_type') is not None else 'пустой').strip()[:80] or 'пустой', str(m.get('description') or '')[:300], i))
    ...
    def move_inventory_item(self, item_id: int, direction: str) -> list[dict[str, Any]]:
        ...
        rows = self._many("SELECT id, sort_order FROM inventory_items WHERE character_id=? ORDER BY sort_order DESC, id DESC", (character_id,))
        idx = next((i for i, row in enumerate(rows) if int(row['id']) == int(item_id)), -1)
        if idx < 0:
    ...
    def delete_weapon_shell_stock(self, item_id: int, stock_id: int) -> bool:
        stock = self._one("SELECT id FROM weapon_shell_stocks WHERE id=? AND item_id=?", (int(stock_id), int(item_id)))
        if not stock:
            return False
        self.conn.execute("DELETE FROM weapon_shell_stocks WHERE id=? AND item_id=?", (int(stock_id), int(item_id)))
        self.conn.commit()
        return True
    ...
    def decide_inventory_request(self, request_id: int, approve: bool) -> dict[str, Any]:
        ...
        if approve:
            ...
            if req['request_type'] == 'reload_weapon' and item:
                ...
                if mag:
                    current = int(mag.get('ammo_current') or 0)
                    max_ammo = int(mag.get('ammo_max') or 0)
                    free = max(0, max_ammo - current)
                    if stock_id:
                        stock = self._one(
                            "SELECT * FROM weapon_shell_stocks WHERE id=? AND item_id=?",
                            (stock_id, int(item['id']) if item else 0),
                        )
                        if not stock:
                            raise ValueError("Патроны не найдены")
                        if current > 0 and str(mag.get('ammo_type') or '') and str(mag.get('ammo_type') or '') != str(stock.get('ammo_type') or ''):
                            raise ValueError("В магазине уже лежат другие патроны")
                        requested = free if amount is None else min(free, max(0, int(amount)))
                        can_load = max(0, min(requested, int(stock.get('quantity') or 0)))
                        if can_load <= 0:
                            raise ValueError("Некуда заряжать или патроны закончились")
                        self.conn.execute(
                            "UPDATE weapon_magazines SET ammo_current=?, ammo_type=?, description=? WHERE id=?",
                            (
                                current + can_load,
                                str(stock.get('ammo_type') or 'патроны')[:80],
                                str(stock.get('description') or '')[:300],
                                mag_id,
                            ),
                        )
                        self.conn.execute("UPDATE weapon_shell_stocks SET quantity=quantity-? WHERE id=?", (can_load, stock_id))
                    else:
                        target = max_ammo if amount is None else min(max_ammo, current + max(0, int(amount)))
