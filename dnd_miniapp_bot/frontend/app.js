...
const state = {
  ...
  cyberBuilderNodes: null,
  pendingUploads: new Map(),
  pendingActions: new Set(),
  filePickerOpen: false,
  viewEpoch: 0,
  cyberInventoryMode: localStorage.getItem('cyber_inventory_mode') || 'implants',
  cyberInventoryCharacterId: Number(localStorage.getItem('cyber_inventory_character_id') || 0),
  cyberInventoryWeaponId: Number(localStorage.getItem('cyber_inventory_weapon_id') || 0),
...
function createUploadId(prefix='upload') {
...
function endFileInteraction() {
  if (!state.pendingUploads.size) state.filePickerOpen = false;
}

window.addEventListener('focus', () => {
  setTimeout(() => {
    if (!document.getElementById('modal') && !state.pendingUploads.size) endFileInteraction();
  }, 800);
});

function uiRefreshLocked() {
  return state.filePickerOpen || state.pendingUploads.size > 0 || !!document.getElementById('modal') || userIsEditing();
}

async function singleFlightUpload(uploadId, operation) {
  const key = String(uploadId || createUploadId());
  if (state.pendingUploads.has(key)) return state.pendingUploads.get(key);
  state.filePickerOpen = true;
  state.viewEpoch += 1;
  ...
  state.pendingUploads.set(key, promise);
  return promise;
}

async function fetchUploadWithRetry(path, form, uploadId, attempts=3) {
  let lastError = null;
  for (let attempt=1; attempt<=attempts; attempt++) {
    try {
      const headers = authUploadHeaders();
      if (uploadId) headers['X-Upload-Id'] = String(uploadId);
      const res = await fetch(path, {method:'POST', headers, body:form});
      let data = {}; try { data = await res.json(); } catch (_) {}
      if (!res.ok) {
        const error = new Error(data.detail || data.message || `Ошибка ${res.status}`);
...
async function apiUpload(path, fileInputId, uploadId='') {
  const input = document.getElementById(fileInputId);
  const file = input?.files?.[0];
  if (!file) throw new Error('Выбери файл изображения');
  const form = new FormData();
  form.append('file', file);
  const id = uploadId || createUploadId('file');
  return singleFlightUpload(id, () => fetchUploadWithRetry(path, form, id));
}

async function apiUploadForm(path, fileInputId, fields={}, uploadId='') {
  const input = document.getElementById(fileInputId);
  const file = input?.files?.[0];
  if (!file) throw new Error('Выбери файл изображения');
  const form = new FormData();
  ...
  const id = uploadId || createUploadId('form');
  return singleFlightUpload(id, () => fetchUploadWithRetry(path, form, id));
}
...
function bindDesktopInputFocusFix() {
  document.querySelectorAll('input, textarea, select').forEach(el => {
    if (el.dataset.focusFixBound === '1') return;
    el.dataset.focusFixBound = '1';
    el.addEventListener('pointerdown', () => {
      setTimeout(() => { try { el.focus({preventScroll:true}); } catch(_) { try { el.focus(); } catch(__) {} } }, 0);
    }, {passive:true});
    el.addEventListener('click', () => {
      setTimeout(() => { try { if (document.activeElement !== el) el.focus({preventScroll:true}); } catch(_) {} }, 0);
    }, {passive:true});
  });
}
...
function bindAfterRender() {
  ...
  document.querySelectorAll('input[type="file"]').forEach(el => {
    if (el.dataset.fileReliabilityBound === '1') return;
    el.dataset.fileReliabilityBound = '1';
    el.addEventListener('click', beginFileInteraction);
    el.addEventListener('change', () => {
      state.viewEpoch += 1;
      if (!el.files?.length) endFileInteraction();
    });
  });

  bindDesktopInputFocusFix();
    ...
    <div class="home-accordion">
      <details class="card stack home-section create-card">
        ...
        <div class="stack home-section-body">
          ...
          <label>Система<select id="newRuleType"><option value="fantasy">Фэнтези</option><option value="cyberpunk">Киберпанк</option></select></label>
...
async function createCampaign() {
  try {
    ...
    const out = await api('/api/campaigns', {method:'POST', body:{name, rule_type:val('newRuleType') || 'fantasy', injuries_enabled:checked('newInj'), armor_enabled:checked('newArmor'), weapons_enabled:checked('newWeapons')}});
...
function renderCampaign() {
  ...
  if (state.tab === 'netrunner') body = renderNetrunner();
  if (state.tab === 'netrunner_master') body = renderMasterNetrunner();
  if (state.tab === 'cyber_inventory') body = renderCyberInventory();
...
function mergeCharacterState(character) {
  if (!character || !state.campaignState) return;
  const replace = list => Array.isArray(list) ? list.map(ch => Number(ch.id)===Number(character.id) ? character : ch) : list;
  state.campaignState.characters = replace(state.campaignState.characters);
  state.campaignState.team_characters = replace(state.campaignState.team_characters);
  if (Number(state.campaignState.player_character?.id) === Number(character.id)) state.campaignState.player_character = character;
}
...
async function openUserProfile(telegramId, focusCharacterId=0) {
  ...
  try {
    ...
    const focusedId = Number(focusCharacterId || 0);
    const currentCampaignId = Number(state.currentCampaignId || state.campaignState?.campaign?.id || 0);
    const main =
      chars.find(ch => Number(ch.id) === focusedId) ||
      chars.find(ch => currentCampaignId && Number(ch.campaign_id) === currentCampaignId) ||
    ...
    const charList = chars.map(ch => `<button type="button" class="profile-line profile-character-line ${Number(ch.id) === Number(main.id) ? 'selected' : ''}" style="--char-color:${esc(ch.color || '#72a7ff')}" onclick="openUserProfile(${Number(telegramId)}, ${Number(ch.id)})">${avatarCircle(ch, 'xs')}<b>${esc(ch.name)}</b>${characterTagPill(ch)}<small>HP ${esc(ch.current_hp)}/${esc(ch.current_max_hp)}</small></button>`).join('') || '<div class="muted">Персонажей нет.</div>';
    const campList = camps.map(c => `<span class="tag-pill ${Number(main.campaign_id) === Number(c.id) ? 'selected' : ''}">${esc(c.emoji || '🎲')} ${esc(c.name)}</span>`).join('') || '<span class="muted">Кампаний нет.</span>';
...
function renderParty() {
  ...
  return `<div class="card stack"><div class="name">🧭 Отряд</div><div class="muted">Нажми на персонажа, чтобы открыть профиль игрока.</div><div class="party-grid">${team.map(ch => `<button class="party-card party-card-button" style="--char-color:${esc(ch.color || '#72a7ff')}" onclick="openUserProfile(${Number(ch.telegram_user_id || 0)}, ${Number(ch.id || 0)})">${avatarCircle(ch, 'md')}<div class="party-main"><b>${esc(ch.name)}</b>${characterTagPill(ch)}<span class="${hpClass(ch)}">${hpText(ch)}</span></div><span class="badge ${ch.injury_count ? 'danger':'ok'}">травмы: ${ch.injury_count || 'нет'}</span></button>`).join('')}</div></div>`;
}
...
function renderCampaignSettingsCard() {
  ...
  const campaigns = state.me?.campaigns || [];
  const options = campaigns.map(x => `<option value="${x.id}" ${x.id===state.currentCampaignId?'selected':''}>${esc(x.emoji || '🎲')} ${esc(x.name)} — ${x.role === 'master' ? 'мастер' : 'игрок'}</option>`).join('');
  const editBlock = isMaster ? `<div class="campaign-edit-box">
      <div class="grid two">
        <label>Иконка кампании<input id="campaignEmojiInput" value="${esc(c.emoji || '🎲')}" maxlength="8" inputmode="text"></label>
        <label>Название кампании<input id="campaignNameInput" value="${esc(c.name || '')}" maxlength="80"></label>
        <label>Система кампании<select id="campaignRuleType"><option value="fantasy" ${c.rule_type==='fantasy'?'selected':''}>Фэнтези</option><option value="cyberpunk" ${c.rule_type==='cyberpunk'?'selected':''}>Киберпанк</option></select></label>
      </div>
      <button class="ok" onclick="saveCampaignSettings()">Сохранить настройки кампании</button>
    </div>` : '';
  return `<details class="card stack campaign-settings-card custom-collapse" ${sectionAttrs('campaign-info-settings')}>
    <summary><div class="unique-custom-head"><span>${esc(c.emoji || '🎛')}</span><div><b>${esc(c.name || 'Кампания')}</b><small>Информация о кампании, режим окна и переключение</small></div></div></summary>
...
async function saveCampaignSettings() {
  try {
    const name = val('campaignNameInput');
    const emoji = val('campaignEmojiInput') || '🎲';
    const rule_type = val('campaignRuleType') || 'fantasy';
    if (!name) throw new Error('Введите название кампании');
    const out = await api(`/api/campaigns/${state.currentCampaignId}`, {method:'PATCH', body:{name, emoji, rule_type}});
    state.campaignState.campaign = out.campaign || state.campaignState.campaign;
    state.me = await api('/api/me');
    showToast('Настройки кампании сохранены', 'heal');
    renderCampaign();
  } catch(e) { showToast(e.message, 'error'); }
}
...
function renderSparkAdminPlayers(sm) {
  const players = sm.players || [];
  const achievements = sm.achievements || state.campaignState?.achievement_templates || [];
  if (!players.length) return '<div class="muted">Привязанных игроков пока нет.</div>';
  const achOptions = achievements.map(a => `<option value="${Number(a.id)}">${esc(a.title || 'Достижение')}</option>`).join('');
  return `<div class="spark-player-list">${players.map(p => {
    const tg = Number(p.telegram_user_id || 0);
    const chars = p.characters || [];
    const charOptions = chars.map(ch => `<option value="${Number(ch.id)}">${esc(ch.name)} · ${esc(ch.campaign_name || '')}</option>`).join('');
    const grants = p.achievement_grants || [];
    const grantsHtml = grants.length ? grants.map(g => `<div class="admin-player-ach-row"><span>${achIconHtml(g.achievement?.icon || '🏆', '', g.achievement?.icon_thumb || '')}</span><div><b>${esc(g.achievement?.title || 'Достижение')}</b><small>${esc(g.created_at_msk || '')}</small></div><button class="small danger" onclick="adminRevokePlayerAchievement(${tg}, ${Number(g.achievement_id || g.achievement?.id || 0)})">Снять</button></div>`).join('') : '<div class="muted">Достижений пока нет.</div>';
    const charNames = chars.map(ch => `${ch.name}${ch.campaign_name ? ' · '+ch.campaign_name : ''}`).join(' / ');
    return `<details class="spark-player-card custom-collapse" ${sectionAttrs('spark-player-'+tg)}>
      <summary><div><b>${esc(p.display_name || ('Игрок TG '+tg))}</b><small>TG ${esc(tg)}${charNames ? ' · '+esc(charNames) : ''}</small></div><span class="spark-balance-pill">✦ ${esc(p.currency_balance || 0)}</span></summary>
      <div class="custom-collapse-body stack">
        <div class="grid two"><label>Изменить искры<input id="adminPlayerSpark_${tg}" type="number" value="100"></label><label>Комментарий<input id="adminPlayerSparkComment_${tg}" placeholder="Корректировка главным мастером"></label></div>
        <button class="ok small" onclick="adminAdjustPlayerSparks(${tg})">Изменить баланс игрока</button>
        <div class="hr"></div>
        <div class="grid two"><label>Достижение<select id="adminPlayerAch_${tg}">${achOptions || '<option value="">Нет достижений</option>'}</select></label><label>Персонаж<select id="adminPlayerChar_${tg}">${charOptions || '<option value="">Нет персонажей</option>'}</select></label><label class="wide">Комментарий<input id="adminPlayerAchComment_${tg}" placeholder="За что выдано"></label></div>
        <button class="ok small" onclick="adminGrantPlayerAchievement(${tg})">Выдать достижение</button>
        <div class="spark-subtitle">Достижения игрока</div>${grantsHtml}
      </div>
    </details>`;
  }).join('')}</div>`;
}
...
function renderSparkManagementCard() {
  ...
  return `<details class="card stack spark-management-card custom-collapse" ${sectionAttrs('spark-management')}>
    ...
    <div class="custom-collapse-body stack">
      ...
      ${isAdmin ? `<details class="custom-section nested-shop" ${sectionAttrs('spark-admin')}><summary><b>👑 Главное управление запасами</b><small>Все мастера, их баланс и история</small></summary>${renderSparkAdminMasters(sm)}</details><details class="custom-section nested-shop" ${sectionAttrs('spark-admin-players')}><summary><b>🧙 Игроки и награды</b><small>Баланс искр, выданные достижения и ручная корректировка</small></summary>${renderSparkAdminPlayers(sm)}</details>` : ''}
    </div>
  </details>`;
}
...
async function adminAdjustPlayerSparks(tg) {
  try {
    const amount = Number(val(`adminPlayerSpark_${tg}`) || 0);
    if (!amount) throw new Error('Укажи количество искр');
    const out = await api(`/api/sparks/admin/players/${tg}/currency`, {method:'POST', body:{amount, comment:val(`adminPlayerSparkComment_${tg}`)}});
    state.campaignState.spark_management = out.spark_management || state.campaignState.spark_management;
    showToast('Баланс игрока изменён', 'heal');
    renderCampaign();
  } catch(e) { showToast(e.message, 'error'); }
}
async function adminGrantPlayerAchievement(tg) {
  try {
    const achievement_id = Number(val(`adminPlayerAch_${tg}`) || 0);
    const character_id = Number(val(`adminPlayerChar_${tg}`) || 0);
    if (!achievement_id) throw new Error('Выбери достижение');
    const out = await api(`/api/sparks/admin/players/${tg}/achievements/grant`, {method:'POST', body:{achievement_id, character_id: character_id || null, master_comment: val(`adminPlayerAchComment_${tg}`)}});
    state.campaignState.spark_management = out.spark_management || state.campaignState.spark_management;
    showToast('Достижение выдано', 'heal');
    renderCampaign();
  } catch(e) { showToast(e.message, 'error'); }
}
async function adminRevokePlayerAchievement(tg, achievementId) {
  try {
    if (!confirm('Снять это достижение у игрока?')) return;
    const out = await api(`/api/sparks/admin/players/${tg}/achievements/${achievementId}`, {method:'DELETE'});
    state.campaignState.spark_management = out.spark_management || state.campaignState.spark_management;
    showToast('Достижение снято', 'info');
    renderCampaign();
  } catch(e) { showToast(e.message, 'error'); }
}

function renderGeneratorSettingsCard() {
  return `<details class="card stack custom-collapse generator-settings-card" ${sectionAttrs('settings-generators')}>
    <summary><div class="unique-custom-head"><span>🎲</span><div><b>События и настроение</b><small>Генераторы настроения, погоды и событий недели</small></div></div></summary>
    <div class="custom-collapse-body">${renderGenerators()}</div>
  </details>`;
}
...
function cyberDefaultNodes() {
  return [
    {id:'gate',level:1,parentId:'root',label:'Внешний шлюз',type:'empty',protected:false,dc:10,children:['cams','ice']},
    {id:'cams',level:2,parentId:'gate',label:'Камеры комплекса',type:'access',protected:true,dc:13,children:['vault']},
    {id:'ice',level:2,parentId:'gate',label:'Сторожевой ICE',type:'guardian',protected:false,dc:12,guardian:{name:'ICE-сторож',hp:14,maxHp:14,ac:12},children:[]},
    {id:'vault',level:3,parentId:'cams',label:'Сейф данных',type:'data',protected:true,dc:16,children:[]},
  ];
}
function cyberNetworkPresets() {
  const node = (id, level, parentId, label, type='empty', dc=10) => {
    const item = {id, level, parentId, label, type, protected:['access','data','trap'].includes(type), dc, children:[]};
    if (type === 'guardian') item.guardian = {name:'ICE', hp:14, maxHp:14, ac:12};
    return item;
  };
  return [
    {id:'street-market', name:'Ночной рынок', depth:2, regen_every:2, note:'Широкая короткая сеть для быстрой сцены.', nodes:[
      node('market-gate',1,'root','Глитч-шлюз'), node('vendor-wifi',1,'root','Wi-Fi торговцев','access',11), node('street-cams',1,'root','Уличные камеры','guardian',12), node('courier',1,'root','Курьерский узел','data',12),
      node('cashbox',2,'vendor-wifi','Кассовый архив','data',14), node('watchlist',2,'street-cams','Розыскной список','trap',13), node('dead-drop',2,'courier','Мёртвый дроп','access',13),
    ]},
    {id:'metro-spine', name:'Магистраль метро', depth:4, regen_every:3, note:'Тоннель с развилкой в середине и закрытым диспетчерским ядром.', nodes:[
      node('turnstile',1,'root','Турникеты'), node('dispatch',2,'turnstile','Диспетчерская','access',13), node('platform-a',3,'dispatch','Платформа A','guardian',13), node('platform-b',3,'dispatch','Платформа B','trap',14), node('signal-core',4,'platform-a','Сигнальное ядро','data',16), node('maintenance',4,'platform-b','Сервисный контур','access',15),
    ]},
    {id:'arcology', name:'Аркология «Гелиос»', depth:5, regen_every:4, note:'Вертикальная корпоративная сеть: от вестибюля до верхнего хранилища.', nodes:[
      node('lobby',1,'root','Лобби'), node('security',2,'lobby','Пост охраны','guardian',13), node('elevators',2,'lobby','Лифты','access',12), node('residential',3,'elevators','Жилой сектор','empty',12), node('labs',3,'elevators','Лаборатории','trap',15), node('executive',4,'residential','Этаж директоров','access',16), node('bio-lock',4,'labs','Биозамок','guardian',16), node('black-vault',5,'executive','Чёрный сейф','data',19), node('gene-archive',5,'bio-lock','Генный архив','data',18),
    ]},
    {id:'ghost-relay', name:'Призрачный ретранслятор', depth:6, regen_every:1, note:'Длинная линейная трасса — идеальна для погони и нарастающего давления.', nodes:[
      node('ping',1,'root','Пинг'), node('echo',2,'ping','Эхо-буфер','trap',12), node('mirror',3,'echo','Зеркальный прокси','guardian',13), node('signal',4,'mirror','Шифросигнал','access',15), node('antenna',5,'signal','Антенна','guardian',16), node('source',6,'antenna','Источник сигнала','data',18),
    ]},
    {id:'city-grid', name:'Городская инфраструктура', depth:3, regen_every:5, note:'Широкая сеть для свободного выбора цели и нескольких параллельных веток.', nodes:[
      node('traffic',1,'root','Трафик'), node('power',1,'root','Энергосеть','guardian',12), node('water',1,'root','Водоканал','access',12), node('med',1,'root','Медсеть','trap',13), node('lights',2,'traffic','Светофоры','access',13), node('substation',2,'power','Подстанция','data',15), node('pumps',2,'water','Насосы','guardian',14), node('triage',2,'med','Триаж','data',15), node('city-admin',3,'lights','Городской админ','data',17), node('backup-grid',3,'substation','Резервная сеть','access',16),
    ]},
    {id:'black-ice-lab', name:'Лаборатория чёрного льда', depth:4, regen_every:2, note:'Пирамидальная боевая схема: чем глубже, тем плотнее защита.', nodes:[
      node('frost-door',1,'root','Морозная дверь'), node('scan-a',2,'frost-door','Сканер A','guardian',13), node('scan-b',2,'frost-door','Сканер B','guardian',13), node('ice-a',3,'scan-a','ICE «Скат»','guardian',15), node('ice-b',3,'scan-a','ICE «Ведьма»','trap',15), node('ice-c',3,'scan-b','ICE «Клык»','guardian',15), node('ice-d',3,'scan-b','ICE «Пиявка»','trap',15), node('kernel',4,'ice-a','Ядро чёрного льда','data',20),
    ]},
  ];
}
function cyberPresetById(id) { return cyberNetworkPresets().find(p => p.id === id) || null; }
function cyberNodeIcon(type) { return ({guardian:'◆',trap:'▲',data:'●',access:'■',empty:'○'})[String(type)] || '○'; }
function renderCyberStructurePreview(nodes, depth) {
  const list = Array.isArray(nodes) ? nodes : [];
  const realDepth = Math.max(Number(depth || 0), ...list.map(n => Number(n.level || 0)), 1);
  const children = id => list.filter(n => String(n.parentId) === String(id)).length;
  return `<div class="cyber-structure-preview" aria-label="Структура сети"><div class="cyber-structure-start">ВХОД</div>${Array.from({length:realDepth}, (_, index) => { const level=index+1, layer=list.filter(n=>Number(n.level)===level); return `<div class="cyber-structure-level"><span class="cyber-structure-floor">${level}</span><div class="cyber-structure-nodes">${layer.length ? layer.map(n => `<span class="cyber-structure-node type-${esc(n.type || 'empty')}" title="${esc(n.label || n.id)}"><i>${cyberNodeIcon(n.type)}</i><b>${esc(n.label || n.id)}</b>${children(n.id)?`<small>↓ ${children(n.id)}</small>`:''}</span>`).join('') : '<span class="cyber-structure-empty">—</span>'}</div></div>`; }).join('')}</div>`;
}
function renderCyberPresetLibrary() {
  return `<div class="cyber-preset-grid">${cyberNetworkPresets().map(p => `<article class="cyber-preset-card"><div><b>${esc(p.name)}</b><small>${p.depth} этажей · ${p.nodes.length} узлов</small></div>${renderCyberStructurePreview(p.nodes, p.depth)}<p>${esc(p.note)}</p><button class="small primary" onclick="cyberInstallPreset('${esc(p.id)}')">Добавить в библиотеку</button></article>`).join('')}</div>`;
}
function cyberBuilderNodes() {
  if (!Array.isArray(state.cyberBuilderNodes)) state.cyberBuilderNodes = cyberDefaultNodes();
  return state.cyberBuilderNodes;
}
function renderCyberBuilderNodes(nodes) {
  if (!nodes.length) return '<div class="muted">Добавь первый узел. В качестве родителя узла первого уровня укажи <code>root</code>.</div>';
  return nodes.slice().sort((a,b)=>Number(a.level)-Number(b.level)).map(n => `<div class="cyber-builder-node"><span><b>${esc(n.label || n.id)}</b><small>${esc(n.id)} · уровень ${esc(n.level)} · родитель ${esc(n.parentId)} · ${esc(n.type)} · DC ${esc(n.dc)}</small></span><button class="small danger" onclick="cyberRemoveNode('${esc(n.id)}')">×</button></div>`).join('');
}
function cyberAddNode() {
  const id = String(val('cyberNodeId') || '').trim().replace(/[^a-zA-Z0-9_-]/g,'_').slice(0,60);
  const parentId = String(val('cyberNodeParent') || 'root').trim();
  const level = Number(val('cyberNodeLevel') || 1);
  if (!id) return showToast('Укажи ID узла','error');
  const nodes = cyberBuilderNodes();
  if (nodes.some(n => n.id === id)) return showToast('Такой ID уже есть','error');
  if (parentId !== 'root' && !nodes.some(n => n.id === parentId)) return showToast('Сначала создай родительский узел','error');
  const type = val('cyberNodeType') || 'empty';
  const node = {id,level:Math.max(1,Math.min(9,level)),parentId,label:String(val('cyberNodeLabel') || id).trim().slice(0,80),type,protected:type!=='empty'&&type!=='guardian',dc:Math.max(1,Math.min(40,Number(val('cyberNodeDc')||12))),children:[]};
  if (type === 'guardian') node.guardian = {name:'ICE',hp:12,maxHp:12,ac:12};
  nodes.push(node);
  nodes.forEach(n => n.children = nodes.filter(x => x.parentId === n.id).map(x => x.id));
  state.cyberBuilderNodes = nodes;
  renderCampaign();
}
function cyberRemoveNode(id) {
  const nodes = cyberBuilderNodes();
  const gone = new Set([id]);
  let changed=true; while(changed){ changed=false; nodes.forEach(n=>{if(gone.has(n.parentId)&&!gone.has(n.id)){gone.add(n.id);changed=true;}}); }
  state.cyberBuilderNodes = nodes.filter(n=>!gone.has(n.id));
  state.cyberBuilderNodes.forEach(n => n.children = state.cyberBuilderNodes.filter(x=>x.parentId===n.id).map(x=>x.id));
  renderCampaign();
}
function cyberSessionControls() {
  const session = cyber()?.session;
  if (!session) return '<div class="muted">Активной сети пока нет.</div>';
  const st = session.state || {}, req = st.hack_request, node = cyberNode(st, st.current_node_id);
  const guardian = node?.guardian;
  return `<div class="cyber-master-live stack">
    <div class="muted">Активно: <b>${esc(session.network?.name || 'Сеть')}</b> · узел: ${esc(node?.label || 'Вход')}</div>
    ${req ? `<div class="cyber-request"><b>Взлом:</b> ${esc(req.label)} · DC ${esc(req.dc)}<div class="row"><button class="ok small" onclick="cyberHackDecision(true)">Подтвердить</button><button class="danger small" onclick="cyberHackDecision(false)">Отклонить</button></div></div>` : ''}
    <div class="grid two"><label>Урон нетраннеру<input id="cyberRunnerDamage" type="number" min="0" value="0"></label><label>Урон скрипту<input id="cyberGuardianDamage" type="number" min="0" value="0" ${guardian?'':'disabled'}></label></div>
    <div class="row"><button class="danger small" onclick="cyberDamage('runner')">Нанести нетраннеру</button><button class="warn small" onclick="cyberDamage('guardian')" ${guardian?'':'disabled'}>Нанести скрипту</button></div>
    <details class="custom-section"><summary><b>DEBUG сети</b></summary><div class="grid two"><label>HP нетраннера<input id="cyberDebugRunnerHp" type="number" value="${esc(st.runner?.hp ?? 0)}"></label><label>NET-щит<input id="cyberDebugRunnerNetHp" type="number" value="${esc(st.runner?.net_hp ?? 0)}"></label><label>HP скрипта<input id="cyberDebugGuardianHp" type="number" value="${esc(guardian?.hp ?? 0)}" ${guardian?'':'disabled'}></label><label>КД скрипта<input id="cyberDebugGuardianAc" type="number" value="${esc(guardian?.ac ?? 10)}" ${guardian?'':'disabled'}></label></div><button class="small" onclick="cyberDebug()">Применить DEBUG</button></details>
  </div>`;
}
function renderCyberMasterSettings() {
  const c = state.campaignState?.campaign || {};
  if (c.rule_type !== 'cyberpunk') return '';
  const info = cyber() || {}, chars = allChars().filter(ch => ch.telegram_user_id);
  const runnerOptions = ['<option value="">Не назначен</option>', ...chars.map(ch => `<option value="${ch.id}" ${Number(info.netrunner_character_id)===Number(ch.id)?'selected':''}>${esc(ch.name)}</option>`)].join('');
  const networks = info.networks || [];
  const draftNodes = cyberBuilderNodes();
  return `<details class="card stack cyber-settings-card custom-collapse" ${sectionAttrs('cyber-network-settings')}>
    <summary><div class="unique-custom-head"><span>🕸️</span><div><b>Сеть нетраннера</b><small>Назначение нетраннера, библиотека сетей и контроль сессии.</small></div></div></summary>
    <div class="custom-collapse-body stack">
      <label>Нетраннер<select id="cyberNetrunnerSelect">${runnerOptions}</select></label><button class="ok small" onclick="saveCyberNetrunner()">Назначить нетраннера</button>
      <div class="hr"></div><b>Готовые сети</b><div class="muted">Выбери шаблон, посмотри его чистую структуру и добавь копию в библиотеку кампании.</div>${renderCyberPresetLibrary()}
      <div class="hr"></div><b>Создать сеть</b><div class="grid two"><label>Название<input id="cyberNetworkName" value="Сеть корпорации"></label><label>Глубина<input id="cyberNetworkDepth" type="number" min="1" max="9" value="3"></label><label>Регенерация<input id="cyberNetworkRegen" type="number" min="0" max="20" value="3"></label></div>
      <div class="cyber-builder"><b>Узлы сети</b><div class="grid two"><label>ID узла<input id="cyberNodeId" placeholder="например, cameras"></label><label>Уровень<input id="cyberNodeLevel" type="number" min="1" max="9" value="1"></label><label>Родитель<input id="cyberNodeParent" value="root" placeholder="root или ID узла"></label><label>Название<input id="cyberNodeLabel" placeholder="Камеры"></label><label>Тип<select id="cyberNodeType"><option value="empty">Развилка</option><option value="guardian">Скрипт</option><option value="trap">Ловушка</option><option value="data">Данные</option><option value="access">Доступ</option></select></label><label>DC<input id="cyberNodeDc" type="number" min="1" max="40" value="12"></label></div><button class="small" onclick="cyberAddNode()">Добавить узел</button><div class="cyber-builder-list">${renderCyberBuilderNodes(draftNodes)}</div></div><button class="ok" onclick="createCyberNetwork()">Добавить сеть в библиотеку</button>
      <div class="hr"></div><b>Библиотека кампании</b>${networks.length ? `<div class="cyber-library-list">${networks.map(n => `<div class="cyber-library-card"><div class="cyber-network-row"><span><b>${esc(n.name)}</b><small>${Number(n.nodes?.length||0)} узлов · глубина ${n.depth}</small></span><button class="primary small" onclick="startCyberNetwork(${n.id})">Запустить</button></div>${renderCyberStructurePreview(n.nodes, n.depth)}</div>`).join('')}</div>` : '<div class="muted">Добавь готовый шаблон или собери первую сеть вручную.</div>'}
      <div class="hr"></div><div class="muted">Запуск и DEBUG активной сети находятся во вкладке 🕸️ в нижнем меню мастера.</div>
    </div>
  </details>`;
}
function cyberNode(st, id) { return (st?.nodes || []).find(n => String(n.id)===String(id)) || (id==='root' ? {id:'root',label:'Точка входа',level:0,children:(st?.nodes||[]).filter(n=>n.parentId==='root').map(n=>n.id)} : null); }
function cyberAdjacent(st, id) { const n=cyberNode(st,id); if(!n) return []; if(id==='root') return (st.nodes||[]).filter(x=>x.parentId==='root').map(x=>x.id); return [...(n.parentId?[n.parentId]:[]), ...(n.children||[])]; }
function cyberNodeTypeName(type) { return ({root:'ВХОД',empty:'РАЗВИЛКА',guardian:'СКРИПТ',trap:'ЛОВУШКА',data:'ДАННЫЕ',access:'ДОСТУП'})[String(type || 'empty')] || 'УЗЕЛ'; }
function renderCyberNetworkMap(st, network, master=false) {
  const current = String(st.current_node_id || 'root');
  const revealed = new Set(st.revealed_details || ['root']);
  const nodes = st.nodes || [];
  const depth = Math.max(Number(network?.depth || 0), ...nodes.map(n => Number(n.level || 0)), 1);
  const adjacent = cyberAdjacent(st, current);
  const nodeHtml = (node, isRoot=false) => {
    const id = String(node.id), isCurrent=id===current, available=!master && !isCurrent && adjacent.includes(id);
    const known = master || isRoot || revealed.has(id) || adjacent.includes(id);
    const type = isRoot ? 'root' : (known ? String(node.type || 'empty') : 'fog');
    const title = known || isRoot ? (node.label || node.id) : 'Неизвестный блок';
    const sub = master
      ? `${cyberNodeTypeName(node.type)} · ${node.protected?'защищён':'открыт'} · DC ${node.dc || 10}${node.guardian ? ` · HP ${node.guardian.hp}/${node.guardian.maxHp || node.guardian.hp}` : ''}`
      : isRoot ? 'Точка входа в сеть' : known ? `${node.protected?'защищён · ':'открыт · '}DC ${node.dc || 10}` : 'Гличный туман';
    return `<button type="button" class="cyber-map-node ${esc(type)} ${isCurrent?'current':''} ${available?'available':''}" data-cyber-node="${esc(id)}" data-cyber-parent="${esc(isRoot ? '' : (node.parentId || 'root'))}" ${available?`onclick="cyberMove('${esc(id)}')"`:''}><div class="cyber-map-node-title"><b>${esc(title)}</b><span>${isRoot?'⌂':cyberNodeIcon(node.type)} ${esc(master || known ? cyberNodeTypeName(node.type) : '???')}</span></div><small>${esc(sub)}</small></button>`;
  };
  const root = {id:'root',label:'Точка входа',type:'root',protected:false,dc:0};
  const levels = Array.from({length:depth}, (_, index) => { const level=index+1, layer=nodes.filter(n=>Number(n.level)===level); return `<div class="cyber-map-level"><span class="cyber-map-caption">Уровень ${level}</span><div class="cyber-map-row">${layer.map(n=>nodeHtml(n)).join('') || '<span class="muted">Пустой уровень</span>'}</div></div>`; }).join('');
  return `<div class="cyber-network-stage ${master?'master-map':''}"><svg class="cyber-network-lines" aria-hidden="true"></svg><div class="cyber-network-tree"><div class="cyber-map-level root-level"><span class="cyber-map-caption">Вход</span><div class="cyber-map-row">${nodeHtml(root, true)}</div></div>${levels}</div></div>`;
}
function drawCyberNetworkLines() {
  document.querySelectorAll('.cyber-network-stage').forEach(stage => {
    const svg = stage.querySelector('.cyber-network-lines');
    if (!svg) return;
    const cards = Array.from(stage.querySelectorAll('[data-cyber-node]'));
    const byId = id => cards.find(el => el.dataset.cyberNode === String(id));
    const rect = stage.getBoundingClientRect(), scrollLeft=stage.scrollLeft, scrollTop=stage.scrollTop;
    const width=Math.max(stage.scrollWidth,stage.clientWidth), height=Math.max(stage.scrollHeight,stage.clientHeight);
    svg.setAttribute('viewBox', `0 0 ${width} ${height}`); svg.setAttribute('width', width); svg.setAttribute('height', height); svg.innerHTML='';
    cards.forEach(to => {
      const parent=to.dataset.cyberParent; if (!parent) return;
      const from=byId(parent); if (!from) return;
      const a=from.getBoundingClientRect(), b=to.getBoundingClientRect();
      const x1=a.left-rect.left+a.width/2+scrollLeft, y1=a.top-rect.top+a.height+scrollTop, x2=b.left-rect.left+b.width/2+scrollLeft, y2=b.top-rect.top+scrollTop, mid=(y1+y2)/2;
      const path=document.createElementNS('http://www.w3.org/2000/svg','path');
      path.setAttribute('d',`M ${x1} ${y1} C ${x1} ${mid}, ${x2} ${mid}, ${x2} ${y2}`);
      path.setAttribute('class', `cyber-network-edge ${(to.classList.contains('available') || from.classList.contains('current'))?'available-edge':''} ${(to.classList.contains('fog') || from.classList.contains('fog'))?'fog-edge':''}`);
      svg.appendChild(path);
    });
  });
}
function renderNetrunner() {
  const info = cyber(), session = info?.session;
  if (!info?.is_netrunner) return '<div class="card muted">Этот раздел доступен назначенному нетраннеру.</div>';
  if (!session) return '<div class="card stack cyber-screen"><div class="name">🕸️ Сеть нетраннера</div><div class="muted">Мастер ещё не запустил сеть.</div></div>';
  const st = session.state || {}, current = String(st.current_node_id || 'root'), node=cyberNode(st,current), req=st.hack_request;
  return `<div class="card stack cyber-screen cyber-session-screen"><div class="unique-custom-head"><span>🕸️</span><div><b>${esc(session.network?.name || 'Сеть')}</b><small>${req?'Взлом ожидает решения мастера':'Жёлтые маршруты доступны для перехода'}</small></div></div><div class="cyber-stats"><span>HP ${esc(st.runner?.hp||0)}/${esc(st.runner?.max_hp||0)}</span><span>NET ${esc(st.runner?.net_hp||0)}/${esc(st.runner?.max_net_hp||0)}</span><span>Действия ${esc(st.actions_left||0)}/${esc(st.actions_max||0)}</span></div>${renderCyberNetworkMap(st, session.network, false)}<div class="row"><button class="ok" onclick="cyberHack()" ${req||!node?.protected?'disabled':''}>Начать взлом</button></div>${req?`<div class="cyber-pending">⌁ Взлом «${esc(req.label)}» — PENDING</div>`:''}<div class="log">${(st.log||[]).slice(-20).reverse().map(x=>`<div class="log-entry">${esc(x)}</div>`).join('')}</div></div>`;
}
function renderMasterNetrunner() {
  const info=cyber(), session=info?.session;
  if (!session) return '<div class="card stack cyber-screen"><div class="name">🕸️ Управление сетью</div><div class="muted">Сначала назначь нетраннера, добавь сеть в библиотеку и запусти её в настройках кампании.</div></div>';
  const st=session.state || {};
  return `<div class="card stack cyber-screen cyber-session-screen cyber-master-screen"><div class="unique-custom-head"><span>🕸️</span><div><b>${esc(session.network?.name || 'Сеть')}</b><small>Мастерский обзор и DEBUG: структура раскрыта полностью.</small></div></div><div class="cyber-stats"><span>Нетраннер HP ${esc(st.runner?.hp||0)}/${esc(st.runner?.max_hp||0)}</span><span>NET ${esc(st.runner?.net_hp||0)}/${esc(st.runner?.max_net_hp||0)}</span><span>Действия ${esc(st.actions_left||0)}/${esc(st.actions_max||0)}</span></div>${renderCyberNetworkMap(st, session.network, true)}${cyberSessionControls()}</div>`;
}
async function cyberRefresh(out) { state.campaignState.cyber = {...(state.campaignState.cyber||{}), ...(out||{})}; await refreshAll(); }
async function saveCyberNetrunner(){ try { await api(`/api/campaigns/${state.currentCampaignId}/cyber/netrunner`,{method:'POST',body:{character_id:Number(val('cyberNetrunnerSelect'))||null}}); await refreshAll(); showToast('Нетраннер назначен','heal'); } catch(e){showToast(e.message,'error');} }
async function cyberInstallPreset(id){ const preset=cyberPresetById(id); if(!preset) return showToast('Шаблон сети не найден','error'); try { await api(`/api/campaigns/${state.currentCampaignId}/cyber/networks`,{method:'POST',body:{name:preset.name,depth:preset.depth,regen_every:preset.regen_every,nodes:preset.nodes}}); await refreshAll(); showToast('Сеть добавлена в библиотеку','heal'); } catch(e){showToast('Не удалось добавить шаблон: '+e.message,'error');} }
async function createCyberNetwork(){ try { const nodes=cyberBuilderNodes(); if(!nodes.length) throw new Error('Добавь хотя бы один узел'); await api(`/api/campaigns/${state.currentCampaignId}/cyber/networks`,{method:'POST',body:{name:val('cyberNetworkName'),depth:Number(val('cyberNetworkDepth')||3),regen_every:Number(val('cyberNetworkRegen')||0),nodes}}); state.cyberBuilderNodes = null; await refreshAll(); showToast('Сеть добавлена в библиотеку','heal'); } catch(e){showToast('Не удалось создать сеть: '+e.message,'error');} }
async function startCyberNetwork(id){ try { await api(`/api/campaigns/${state.currentCampaignId}/cyber/session/start`,{method:'POST',body:{network_id:Number(id)}}); await refreshAll(); showToast('Сеть запущена','heal'); } catch(e){showToast(e.message,'error');} }
async function cyberMove(id){ try { await api(`/api/campaigns/${state.currentCampaignId}/cyber/move`,{method:'POST',body:{node_id:id}}); await refreshAll(); } catch(e){showToast(e.message,'error');} }
async function cyberHack(){ try { await api(`/api/campaigns/${state.currentCampaignId}/cyber/hack`,{method:'POST'}); await refreshAll(); } catch(e){showToast(e.message,'error');} }
async function cyberHackDecision(approve){ try { await api(`/api/campaigns/${state.currentCampaignId}/cyber/hack-decision`,{method:'POST',body:{approve}}); await refreshAll(); } catch(e){showToast(e.message,'error');} }
...
const CYBER_INV_MODES = {
  implants: [
    ...
    { id:'brain-2', label:'Мозг 2', x:50, y:5, accepts:['brain','cyber'], lineX:50.407827833075324, lineY:27.266665649414062 },
    { id:'brain-3', label:'Мозг 3', x:85, y:5, accepts:['brain','cyber'], lineX:50.407827833075324, lineY:27.266665649414062 },
    { id:'eyes', label:'Глаза', x:15, y:25, accepts:['eyes','cyber'], lineX:49.429035059617554, lineY:30.066665649414066 },
    { id:'arms', label:'Руки', x:85, y:25, accepts:['arms','cyber'], lineX:60, lineY:50 },
    { id:'palms', label:'Ладони', x:85, y:40, accepts:['palms','cyber'], lineX:63.132133888026374, lineY:56.46666564941406 },
    { id:'body', label:'Тело', x:15, y:45, accepts:['body','cyber'], lineX:45, lineY:40 },
    { id:'skin', label:'Кожа', x:15, y:60, accepts:['skin','cyber'], lineX:45, lineY:60 },
    { id:'organ-1', label:'Орган 1', x:85, y:70, accepts:['organ','cyber'], lineX:50, lineY:50 },
    { id:'organ-2', label:'Орган 2', x:85, y:85, accepts:['organ','cyber'], lineX:50, lineY:50 },
    { id:'legs', label:'Ноги', x:15, y:85, accepts:['legs','cyber'], lineX:50, lineY:75 }
  ],
  gear: [
    { id:'extra-1', label:'Доп 1', x:15, y:5, accepts:['equipment','cyber'], hideLine:true },
    { id:'extra-2', label:'Доп 2', x:50, y:5, accepts:['equipment','cyber'], hideLine:true },
    { id:'extra-3', label:'Доп 3', x:85, y:5, accepts:['equipment','cyber'], hideLine:true },
    { id:'armor', label:'Броня', x:15, y:30, accepts:['armor'], lineX:50, lineY:47 },
...
function cyberInventoryCharacters() {
  if (state.campaignState?.role === 'master') return allChars();
  const ch = playerChar();
  return ch ? [ch] : [];
}
function activeCyberInventoryCharacter() {
  const chars = cyberInventoryCharacters();
  if (!chars.length) return null;
  let ch = chars.find(x => Number(x.id) === Number(state.cyberInventoryCharacterId));
  if (!ch) ch = chars[0];
  state.cyberInventoryCharacterId = Number(ch.id);
  localStorage.setItem('cyber_inventory_character_id', String(ch.id));
  return ch;
}
function setCyberInventoryCharacter(id) {
  state.cyberInventoryCharacterId = Number(id || 0);
  localStorage.setItem('cyber_inventory_character_id', String(state.cyberInventoryCharacterId || 0));
  setCyberInventoryWeapon(0, false);
  state.cyberInventoryPickedItemId = 0;
  state.cyberInventorySelectedItemId = 0;
  state.cyberInventorySelectedSlot = null;
  renderCampaign();
}
function setCyberInventoryWeapon(itemId, shouldRender=true) {
  state.cyberInventoryWeaponId = Number(itemId || 0);
  if (state.cyberInventoryWeaponId) localStorage.setItem('cyber_inventory_weapon_id', String(state.cyberInventoryWeaponId));
  else localStorage.removeItem('cyber_inventory_weapon_id');
  state.cyberInventoryPickedItemId = 0;
  state.cyberInventorySelectedItemId = 0;
  state.cyberInventorySelectedSlot = null;
  if (shouldRender) renderCampaign();
}
function setCyberInventoryMode(mode) {
  state.cyberInventoryMode = mode === 'gear' ? 'gear' : 'implants';
  localStorage.setItem('cyber_inventory_mode', state.cyberInventoryMode);
  state.cyberInventoryPickedItemId = 0;
  renderCampaign();
}
function cyberInventorySlotRows(ch) {
  return state.campaignState?.cyber_inventory_slots?.[String(ch?.id)] || [];
}
function cyberInventorySlotMap(ch) {
  const map = {};
  cyberInventorySlotRows(ch).forEach(s => { if (s?.item_id) map[`${s.mode}:${s.slot_id}`] = Number(s.item_id); });
  return map;
}
function cyberItemById(ch, itemId) {
  return (currentInventory(ch) || []).find(it => Number(it.id) === Number(itemId)) || null;
}
...
function cyberItemKind(it) {
  ...
  if (/одеж|cloth|костюм|рубаш|плащ/.test(text)) return 'clothes';
  if (/глаз|optic|оптик|зрен/.test(text)) return 'eyes';
  if (/мозг|нейро|neuro|процессор|cortex/.test(text)) return 'brain';
  if (/орган|серд|л[её]гк|печен|киберсерд|organ|lung|heart/.test(text)) return 'organ';
  if (/ладон|кисть|palm|hand/.test(text)) return 'palms';
  if (/рук|arm|мантис|клинк/.test(text)) return 'arms';
  if (/кожа|skin|подкож/.test(text)) return 'skin';
  if (/нога|leg|ступн/.test(text)) return 'legs';
  if (/тело|body|корпус|torso/.test(text)) return 'body';
  return 'cyber';
}
...
function cyberSlotById(mode, slotId) {
  return (CYBER_INV_MODES[mode] || []).find(s => s.id === slotId) || null;
}
function cyberInventoryBagEntries(ch) {
  return (currentInventory(ch) || []).map(it => ({kind:'item', item:it}));
}
function cyberWeaponAmmoText(it) {
  if (!it || it.item_type !== 'weapon') return '';
  if (it.reload_type === 'shell') {
    const loaded = (it.loaded_shells || []).length;
    return `${loaded}/${Number(it.mag_capacity || 0) || 0}`;
  }
  const mag = (it.magazines || []).find(m => Number(m.id) === Number(it.active_magazine_id));
  if (!mag) return 'нет маг.';
  return `${Number(mag.ammo_current || 0)}/${Number(mag.ammo_max || 0)}`;
}
function cyberWeaponInventoryEntries(weapon) {
  if (!weapon) return [];
  const entries = [];
  const activeMagId = Number(weapon.active_magazine_id || 0);
  if (weapon.reload_type === 'magazine') {
    (weapon.magazines || [])
      .filter(mag => Number(mag.id) !== activeMagId)
      .forEach(magazine => entries.push({kind:'magazine', weapon, magazine}));
  }
  (weapon.shell_stocks || []).forEach(stock => entries.push({kind:'ammo', weapon, stock}));
  return entries;
}
function cyberWeaponStoredMagazines(weapon) {
  const activeMagId = Number(weapon?.active_magazine_id || 0);
  return (weapon?.magazines || []).filter(mag => Number(mag.id) !== activeMagId);
}
function cyberActiveMagazine(weapon) {
  if (!weapon || weapon.reload_type !== 'magazine') return null;
...
function renderCyberInventorySlots(ch, mode) {
  ...
  const picked = cyberItemById(ch, state.cyberInventoryPickedItemId);
  return (CYBER_INV_MODES[mode] || []).map(slot => {
    ...
    return `<button type="button" class="${cls.join(' ')}" data-cyber-inv-slot="${esc(slot.id)}" data-cyber-inv-mode="${mode}" style="left:${Number(slot.x)}%;top:${Number(slot.y)}%" onclick="cyberInventorySlotTap(${Number(ch.id)}, '${mode}', '${esc(slot.id)}')">
      <span class="cyber-inv-slot-label">${esc(slot.label)}</span>
      <span class="cyber-inv-slot-content">${it ? `<span>${esc(cyberItemIcon(it))}<small>${esc(it.name)}</small>${ammo?`<em>${esc(ammo)}</em>`:''}</span>` : '<i></i>'}</span>
    </button>`;
  }).join('');
}
function renderCyberInventoryBag(ch) {
  const entries = cyberInventoryBagEntries(ch);
  const cells = Math.max(16, Math.ceil(entries.length / 4) * 4);
  let html = '';
  for (let i=0; i<cells; i++) {
    const entry = entries[i];
    html += `<div class="cyber-inv-bag-cell" data-cyber-bag-index="${i}">${entry ? renderCyberInventoryBagItem(ch, entry) : ''}</div>`;
  }
  return html;
}
function renderCyberInventoryBagItem(ch, entry) {
  if (entry.kind === 'magazine') {
    const mag = entry.magazine || {};
    const weapon = entry.weapon || {};
    const active = Number(mag.id) === Number(weapon.active_magazine_id);
    return `<button type="button" class="cyber-inv-bag-item cyber-inv-virtual-item ${active?'equipped':''}" onclick="openMagazineModal(${Number(weapon.id)}, ${Number(mag.id)})">
      ${active ? '<span class="cyber-inv-equipped-badge">✓</span>' : ''}
      <span class="cyber-inv-bag-icon">▣</span>
      <span class="cyber-inv-bag-name">${esc(mag.name || 'Магазин')}</span>
      <span class="cyber-inv-qty">${esc(mag.ammo_current || 0)}/${esc(mag.ammo_max || 0)}</span>
    </button>`;
  }
  if (entry.kind === 'ammo') {
    const stock = entry.stock || {};
    const weapon = entry.weapon || {};
    return `<button type="button" class="cyber-inv-bag-item cyber-inv-virtual-item ammo" onclick="openShellStockModal(${Number(weapon.id)}, ${Number(stock.id)})">
      <span class="cyber-inv-bag-icon">${esc(stock.emoji || '⦿')}</span>
      <span class="cyber-inv-bag-name">${esc(stock.ammo_type || 'Патроны')}</span>
      <span class="cyber-inv-qty">${esc(stock.quantity || 0)}</span>
    </button>`;
  }
  const it = entry.item;
  const equipped = cyberEquippedSlotForItem(ch, it.id);
  const selected = Number(state.cyberInventorySelectedItemId) === Number(it.id);
  const picked = Number(state.cyberInventoryPickedItemId) === Number(it.id);
  return `<button type="button" class="cyber-inv-bag-item ${equipped?'equipped':''} ${selected||picked?'selected':''}" data-cyber-item="${Number(it.id)}" data-character-id="${Number(ch.id)}">
...
function renderCyberWeaponInventory(ch, weapon) {
  const chars = cyberInventoryCharacters();
  const picker = state.campaignState?.role === 'master' ? `<select class="cyber-inv-character-select" onchange="setCyberInventoryCharacter(this.value)">${chars.map(c => `<option value="${c.id}" ${Number(c.id)===Number(ch.id)?'selected':''}>${esc(c.name)}</option>`).join('')}</select>` : '';
  const entries = cyberWeaponInventoryEntries(weapon);
  const cells = Math.max(12, Math.ceil(entries.length / 4) * 4);
  let bag = '';
  for (let i=0; i<cells; i++) bag += `<div class="cyber-inv-bag-cell">${entries[i] ? renderCyberWeaponInventoryItem(weapon, entries[i]) : ''}</div>`;
  const active = cyberActiveMagazine(weapon);
  const status = weapon.reload_type === 'shell'
    ? `В оружии: ${(weapon.loaded_shells || []).length}/${Number(weapon.mag_capacity || 0) || 0}`
    : active ? `В оружии: ${esc(active.name || 'магазин')} · ${esc(active.ammo_current || 0)}/${esc(active.ammo_max || 0)}` : 'В оружии нет магазина';
  const stockCount = (weapon.shell_stocks || []).reduce((sum, st) => sum + Number(st.quantity || 0), 0);
  const magLine = weapon.reload_type === 'magazine' ? ` · магазинов в сумке: ${cyberWeaponStoredMagazines(weapon).length}` : '';
  const createButtons = weapon.reload_type === 'shell'
    ? `<button class="secondary small" onclick="openShellStockCreateModal(${Number(weapon.id)})">➕ Патроны</button>`
    : `<button class="secondary small" onclick="openMagazineCreateModal(${Number(weapon.id)})">➕ Магазин</button><button class="secondary small" onclick="openShellStockCreateModal(${Number(weapon.id)})">➕ Патроны</button>`;
  return `<div class="cyber-inventory-screen cyber-weapon-inventory-screen">
    <div class="cyber-inv-top">
      <div><span class="cyber-inv-eyebrow">weapon inventory</span><div class="name">🔫 ${esc(weapon.name || 'Оружие')}</div></div>
      ${picker}
    </div>
    <section class="cyber-weapon-panel">
      <div class="cyber-weapon-head">
        <button class="secondary small" onclick="setCyberInventoryWeapon(0)">← Назад</button>
        <button class="secondary small" onclick="openWeaponSettings(${Number(weapon.id)})">⚙ Настройки</button>
      </div>
      <div class="cyber-weapon-status">
        <div class="cyber-inv-big-icon">${esc(cyberItemIcon(weapon))}</div>
        <div><b>${esc(status)}</b><small>Патронов в запасе: ${esc(stockCount)}${esc(magLine)}</small></div>
      </div>
      ${renderCyberInventoryWeaponControls(weapon, false)}
    </section>
    <section class="cyber-inv-bag-card">
      <div class="cyber-inv-bag-head">
        <div><b>Инвентарь оружия</b><span>${entries.length} карточек · магазины и патроны принадлежат этой пушке</span></div>
        <div class="cyber-inv-bag-actions">${createButtons}</div>
      </div>
      <div class="cyber-inv-bag-grid">${bag}</div>
    </section>
  </div>`;
}
function renderCyberWeaponInventoryItem(weapon, entry) {
  if (entry.kind === 'magazine') {
    const mag = entry.magazine || {};
    return `<button type="button" class="cyber-inv-bag-item cyber-inv-virtual-item" onclick="openMagazineModal(${Number(weapon.id)}, ${Number(mag.id)})">
      <span class="cyber-inv-bag-icon">▣</span>
      <span class="cyber-inv-bag-name">${esc(mag.name || 'Магазин')}</span>
      <span class="cyber-inv-bag-sub">магазин от ${esc(weapon.name || 'оружия')}</span>
      <span class="cyber-inv-qty">${esc(mag.ammo_current || 0)}/${esc(mag.ammo_max || 0)}</span>
    </button>`;
  }
  const stock = entry.stock || {};
  return `<button type="button" class="cyber-inv-bag-item cyber-inv-virtual-item ammo" onclick="openShellStockModal(${Number(weapon.id)}, ${Number(stock.id)})">
    <span class="cyber-inv-bag-icon">${esc(stock.emoji || '⦿')}</span>
    <span class="cyber-inv-bag-name">${esc(stock.ammo_type || 'Патроны')}</span>
    <span class="cyber-inv-bag-sub">патроны от ${esc(weapon.name || 'оружия')}</span>
    <span class="cyber-inv-qty">${esc(stock.quantity || 0)}</span>
  </button>`;
}
function renderCyberInventorySheet(ch) {
  return '';
  const it = cyberItemById(ch, state.cyberInventorySelectedItemId);
  if (!it) return '';
  const equipped = cyberEquippedSlotForItem(ch, it.id);
  ...
  return `<aside class="cyber-inv-sheet open">
    <div class="cyber-inv-sheet-head">
      ...
      <button onclick="closeCyberInventorySheet()">×</button>
    </div>
    <p>${esc(it.description || 'Описание пока не заполнено.')}</p>
    ${weaponControls}
...
function openCyberInventoryItemModal(characterId, itemId) {
  ...
  showModal(`<div class="modal-card compact cyber-inv-item-modal">
    ...
    <div class="row modal-actions">
      <button class="ok" onclick="closeModal(); pickCyberInventoryItem(${Number(it.id)})">Выбрать</button>
      <button class="secondary" onclick="openItemEditModal(${Number(it.id)})">Изменить</button>
      <button class="danger" ${equipped?'':'disabled'} onclick="closeModal(); cyberUnequipItem(${Number(ch.id)}, '${equipped ? esc(equipped.mode) : 'implants'}', '${equipped ? esc(equipped.slot_id) : ''}')">Снять</button>
    </div>
  </div>`);
}
function openCyberEquippedWeaponModal(characterId, itemId) {
  const ch = findChar(characterId) || activeCyberInventoryCharacter();
  const it = cyberItemById(ch, itemId);
  if (!it) return showToast('Оружие не найдено', 'error');
  showModal(`<div class="modal-card compact cyber-inv-item-modal">
    <button class="modal-close" onclick="closeModal()">×</button>
    <div class="cyber-inv-item-summary modal-summary"><div class="cyber-inv-big-icon">${esc(cyberItemIcon(it))}</div><div><strong>${esc(it.name)}</strong><span>Оружие · ${esc(cyberWeaponAmmoText(it))}</span></div></div>
    ${renderCyberInventoryWeaponControls(it, false)}
    <div class="row modal-actions">
      <button class="secondary" onclick="closeModal(); setCyberInventoryWeapon(${Number(it.id)})">Инвентарь оружия</button>
      <button class="secondary" onclick="closeModal()">Закрыть</button>
    </div>
  </div>`);
}
function renderCyberInventoryWeaponControls(it, withManage=true) {
  const modes = (it.fire_modes||[]).length ? it.fire_modes : [{id:0,name:'Атака',ammo_cost:it.ammo_per_attack||1,description:''}];
  const fire = modes.map(m => `<button class="small cyber-inv-fire-btn" onclick="fireWeapon(${Number(it.id)}, ${Number(m.id || 0)})"><span>${esc(m.name)}</span><b>${esc(m.ammo_cost || 1)}</b></button>`).join('');
  const reload = it.reload_type === 'shell'
    ? `<button class="secondary" onclick="chooseLoadShells(${Number(it.id)})">🔄 Зарядить</button>`
    : `<button class="secondary" onclick="quickReloadWeapon(${Number(it.id)})">🔄 Перезарядить первым магазином</button>`;
  const manage = !withManage ? '' : (it.reload_type === 'shell'
    ? `<div class="cyber-inv-manage-grid"><button class="secondary small" onclick="openShellStockCreateModal(${Number(it.id)})">➕ Патроны</button></div>`
    : `<div class="cyber-inv-manage-grid"><button class="secondary small" onclick="openMagazineCreateModal(${Number(it.id)})">➕ Магазин</button><button class="secondary small" onclick="openShellStockCreateModal(${Number(it.id)})">➕ Патроны</button></div>`);
  return `<div class="cyber-inv-weapon-box"><div class="cyber-inv-fire-grid">${fire}</div>${reload}${manage}</div>`;
}
function closeCyberInventorySheet() {
  state.cyberInventorySelectedItemId = 0;
  state.cyberInventorySelectedSlot = null;
  renderCampaign();
}
function pickCyberInventoryItem(itemId) {
  state.cyberInventoryPickedItemId = Number(itemId || 0);
  state.cyberInventorySelectedItemId = 0;
  renderCampaign();
}
function cyberInventorySlotTap(characterId, mode, slotId) {
  const ch = findChar(characterId) || activeCyberInventoryCharacter();
  const slot = cyberSlotById(mode, slotId);
  const picked = cyberItemById(ch, state.cyberInventoryPickedItemId);
  const slotItemId = cyberInventorySlotMap(ch)[`${mode}:${slotId}`];
  if (picked) {
    if (!cyberCanEquip(picked, slot)) return showToast('Этот предмет не подходит к слоту', 'error');
    return cyberEquipItem(characterId, mode, slotId, picked.id);
  }
  if (slotItemId) {
...
async function cyberEquipItem(characterId, mode, slotId, itemId) {
  try {
    ...
    state.cyberInventoryPickedItemId = 0;
    state.cyberInventorySelectedItemId = Number(itemId);
    showToast('Предмет экипирован', 'heal');
    renderCampaign();
  } catch(e) { showToast(e.message, 'error'); }
}
...
function drawCyberInventoryLines() {
  ...
  (CYBER_INV_MODES[mode] || []).forEach(slot => {
    if (slot.hideLine) return;
    const node = stage.querySelector(`[data-cyber-inv-slot="${slot.id}"]`);
    if (!node) return;
    const nr = node.getBoundingClientRect();
    const x1 = rect.width * Number(slot.lineX || 50) / 100;
    const y1 = rect.height * Number(slot.lineY || 50) / 100;
    const cx = nr.left - rect.left + nr.width / 2;
    const cy = nr.top - rect.top + nr.height / 2;
    const dx = cx - x1, dy = cy - y1, dist = Math.max(1, Math.hypot(dx, dy));
    const pad = Math.min(42, dist * 0.42);
    const line = document.createElementNS('http://www.w3.org/2000/svg', 'line');
    line.setAttribute('x1', x1);
    line.setAttribute('y1', y1);
    line.setAttribute('x2', cx - dx / dist * pad);
    line.setAttribute('y2', cy - dy / dist * pad);
    svg.appendChild(line);
  });
}
function bindCyberInventoryDrag() {
  document.querySelectorAll('.cyber-inv-bag-item').forEach(node => {
    if (node.dataset.boundDrag === '1') return;
    node.dataset.boundDrag = '1';
    node.addEventListener('pointerdown', startCyberInventoryPointer);
  });
}
function startCyberInventoryPointer(event) {
  const node = event.currentTarget;
  const itemId = Number(node.dataset.cyberItem || 0);
  const characterId = Number(node.dataset.characterId || 0);
  ...
  const end = ev => {
    ...
    if (drag.slot) return cyberEquipItem(characterId, drag.slot.mode, drag.slot.slotId, itemId);
...
async function saveCustomFrame(id, frame) {
  const actionKey = `custom-frame-${id}`;
  if (state.pendingActions.has(actionKey)) return;
  state.pendingActions.add(actionKey);
  try {
    state.viewEpoch += 1;
    const out = await api(`/api/characters/${id}/self`, {method:'PATCH', body:{custom_frame: frame}});
    mergeCharacterState(out.character);
    showToast(frame ? 'Сверкающая рамка применена' : 'Рамка отключена', 'heal');
    closeModal();
    renderCampaign();
  } catch(e) { showToast(e.message, 'error'); }
  finally { state.pendingActions.delete(actionKey); }
}


async function saveCustomEffect(id, effect) {
  const actionKey = `custom-effect-${id}`;
  if (state.pendingActions.has(actionKey)) return;
  state.pendingActions.add(actionKey);
  try {
    state.viewEpoch += 1;
    const out = await api(`/api/characters/${id}/self`, {method:'PATCH', body:{custom_effect: effect}});
    mergeCharacterState(out.character);
...
async function saveCroppedAvatar() {
  ...
  crop.saving = true;
  const saveButton = document.getElementById('saveAvatarCropBtn');
  if (saveButton) { saveButton.disabled = true; saveButton.textContent = 'Загрузка…'; }
  try {
    const img = document.getElementById('avatarCropImg');
    ...
    const base = Math.max(size/iw, size/ih) * crop.scale;
    const dw = iw * base, dh = ih * base;
    const dx = (size - dw)/2 + crop.x * (size/320);
    const dy = (size - dh)/2 + crop.y * (size/320);
    ctx.drawImage(img, dx, dy, dw, dh);
    const blob = await new Promise(resolve => canvas.toBlob(resolve, 'image/png', 0.92));
    const form = new FormData();
    form.append('file', blob, 'avatar.png');
    const data = await singleFlightUpload(crop.uploadId, () => fetchUploadWithRetry(`/api/characters/${crop.characterId}/avatar`, form, crop.uploadId));
    mergeCharacterState(data.character);
    closeModal();
    ...
    crop.saving = false;
    endFileInteraction();
    if (saveButton) { saveButton.disabled = false; saveButton.textContent = 'Сохранить аватарку'; }
    showToast(e.message, 'error');
  }
}
...
async function uploadAvatar(id, inputId) {
  try {
    const out = await apiUpload(`/api/characters/${id}/avatar`, inputId);
    mergeCharacterState(out.character);
...
function renderInventoryRows(ch, items) {
  return items.map((it, idx) => {
    const main = renderInventoryItem(ch, it, idx, items.length);
    if (it.item_type !== 'weapon') return main;
    const mags = it.reload_type === 'magazine' ? (it.magazines || []).map(m => renderMagazineInventoryItem(it, m)).join('') : '';
    const ammo = (it.shell_stocks || []).map(st => renderAmmoInventoryItem(it, st)).join('');
    return main + mags + ammo;
  }).join('');
}
...
function renderInventoryItem(ch, it, idx=0, total=1) {
  ...
  return `<div class="inventory-item clickable" onclick="openInventoryItemInfo(${it.id})"><div class="inventory-main">${icon}<div><b>${esc(it.name)}</b><small>Количество: ${esc(it.quantity || 1)}</small>${it.description?`<p>${esc(shortText(it.description, 120))}</p>`:'<p class="muted">Нажми, чтобы открыть описание.</p>'}</div></div><div class="inventory-actions-row" onclick="event.stopPropagation()">${inventoryMoveControls(it.id, idx, total)}<div class="row inventory-edit-actions"><button class="secondary small" onclick="openItemEditModal(${it.id})">Изменить</button><button class="secondary small" onclick="editItemQty(${it.id}, ${it.quantity || 1})">Кол-во</button><button class="danger small" onclick="deleteItem(${it.id})">Удалить</button></div></div></div>`;
}
function shortText(text, max=120) {
  const s = String(text || '').trim();
  return s.length > max ? s.slice(0, max - 1) + '…' : s;
}
function openInventoryItemInfo(itemId) {
  const it = itemById(itemId);
  if (!it) return showToast('Предмет не найден', 'error');
  const icon = it.emoji ? `<div class="item-info-icon">${esc(it.emoji)}</div>` : '<div class="item-info-icon">🎒</div>';
  showModal(`<div class="modal-card compact item-info-modal"><button class="modal-close" onclick="closeModal()">×</button>${icon}<div class="name">${esc(it.name || 'Предмет')}</div><div class="muted centered-text">Количество: ${esc(it.quantity || 1)}</div><p>${esc(it.description || 'Описание не заполнено.')}</p><div class="row modal-actions"><button class="secondary" onclick="closeModal()">Закрыть</button></div></div>`);
}
...
function renderMagazineWeapon(ch, it, icon, idx=0, total=1) {
  ...
  const magsCount = (it.magazines || []).length;
  const ammoCount = (it.shell_stocks || []).reduce((sum, st) => sum + Number(st.quantity || 0), 0);
  return `<div class="inventory-item weapon-item${firingClass}">${weaponHeader(it, icon, 'оружие · магазинная перезарядка', idx, total)}<div class="ammo-line">${activeHtml}</div><div class="muted small">Магазинов: ${magsCount}. Патронов в запасе: ${ammoCount}.</div><div class="weapon-fire-modes">${modeBtns}</div><button class="secondary reload-weapon-btn" onclick="chooseReloadMag(${it.id})">🔄 Выбрать магазин</button></div>`;
}
function renderShellWeapon(ch, it, icon, idx=0, total=1) {
  ...
  const ammoCount = (it.shell_stocks || []).reduce((sum, st) => sum + Number(st.quantity || 0), 0);
  return `<div class="inventory-item weapon-item shotgun-item${firingClass}">${weaponHeader(it, icon, `оружие · поштучная зарядка · ${loaded.length}/${maxLoaded}`, idx, total)}<div class="ammo-line"><span>Заряжено: ${loadedHtml}</span></div><div class="muted small">Патронов в запасе: ${ammoCount}.</div><div class="weapon-fire-modes">${modeBtns}</div><button class="secondary reload-weapon-btn" onclick="chooseLoadShells(${it.id})">🔄 Зарядить</button></div>`;
}
function renderMagazineInventoryItem(weapon, mag) {
  const active = Number(mag.id) === Number(weapon.active_magazine_id);
  const icon = `<span class="item-emoji">▣</span>`;
  return `<div class="inventory-item virtual-inventory-item magazine-inventory-item ${active?'active':''}"><div class="inventory-main">${icon}<div><b>${esc(mag.name || 'Магазин')}</b><small>магазин от ${esc(weapon.name || 'оружия')}</small><p><b>${esc(mag.ammo_current || 0)}/${esc(mag.ammo_max || 0)}</b> ${ammoTypePill(mag.ammo_type || 'пустой', mag.description || '')}${active?' · активный':''}</p></div></div><div class="row inventory-edit-actions"><button class="ok small" onclick="openMagazineModal(${weapon.id}, ${mag.id})">Открыть</button><button class="secondary small" onclick="requestReload(${weapon.id}, ${mag.id})">В оружие</button></div></div>`;
}
function renderAmmoInventoryItem(weapon, stock) {
  const icon = `<span class="item-emoji">${esc(stock.emoji || '⦿')}</span>`;
  const action = weapon.reload_type === 'shell'
    ? `<button class="ok small" onclick="chooseLoadShells(${weapon.id})">Зарядить оружие</button>`
    : `<button class="ok small" onclick="chooseAmmoTargetMagazine(${weapon.id}, ${stock.id})">В магазин</button>`;
  return `<div class="inventory-item virtual-inventory-item ammo-inventory-item"><div class="inventory-main">${icon}<div><b>${esc(stock.ammo_type || 'Патроны')}</b><small>патроны от ${esc(weapon.name || 'оружия')}</small><p>Количество: <b>${esc(stock.quantity || 0)}</b>${stock.description?` · ${esc(stock.description)}`:''}</p></div></div><div class="row inventory-edit-actions">${action}<button class="secondary small" onclick="openShellStockModal(${weapon.id}, ${stock.id})">Открыть</button></div></div>`;
}
function openItemModal(charId, type) {
  const isWeapon = type === 'weapon';
  showModal(`<div class="modal-card ${isWeapon?'weapon-create-modal':''}"><button class="modal-close" onclick="closeModal()">×</button><div class="name">${isWeapon?'🔫 Новое оружие':'🎒 Новый предмет'}</div><label>Название<input id="itemName" placeholder="Например: Револьвер, Дробовик, Винтовка"></label>${isWeapon?'':`<label>Эмоджи<input id="itemEmoji" placeholder="например 🧪"></label>`}<label>Описание<textarea id="itemDesc" placeholder="Коротко опиши предмет"></textarea></label>${isWeapon?`<div class="weapon-create-type-grid"><button type="button" id="reloadMagazineBtn" class="weapon-type-choice active" onclick="setWeaponReloadType('magazine')"><b>С магазинами</b><small>Пистолет, ПП, автомат</small></button><button type="button" id="reloadShellBtn" class="weapon-type-choice" onclick="setWeaponReloadType('shell')"><b>Без магазинов</b><small>Дробовик, винтовка, револьвер</small></button></div><input id="reloadType" type="hidden" value="magazine"><label>Патронов за обычную атаку<input id="ammoPerAttack" type="number" min="1" value="1"></label><div id="magazineFields" class="stack"><label>Ёмкость одного магазина<input id="magCapacity" type="number" min="1" value="30"></label><label>Сколько магазинов добавить в инвентарь<input id="magazineCount" type="number" min="0" max="16" value="2"></label></div><div id="shellFields" class="stack hidden"><label>Сколько максимум помещается в оружие<input id="shellCapacity" type="number" min="1" value="2"></label></div><div class="fire-mode-builder"><div class="fire-mode-head"><div><b>Дополнительные режимы стрельбы</b><small>Обычная атака создаётся сама. Нажми плюс, если нужны очередь, автоогонь или особая атака.</small></div><button type="button" class="secondary add-fire-mode-btn" onclick="addFireModeRow()">＋</button></div><div id="fireModeRows" class="fire-mode-rows"></div></div><small class="muted weapon-create-hint">Оружие создаётся отдельно. Магазины создаются отдельными карточками. Патроны добавишь отдельной стопкой и потом зарядишь ими магазин.</small>`:`<label>Количество<input id="itemQty" type="number" value="1"></label>`}<div class="row modal-actions"><button class="ok" onclick="createItem(${charId}, '${type}')">Добавить</button><button class="secondary" onclick="closeModal()">Отмена</button></div></div>`);
}
...
function setWeaponReloadType(type){
  ...
  document.getElementById('magazineFields')?.classList.toggle('hidden', rt!=='magazine');
...
async function createItem(charId, type) {
  try {
    ...
    if (type==='weapon') {
      ...
      Object.assign(body, {weapon_type:rt==='shell'?'Без магазинов':'Магазинное', reload_type:rt, ammo_per_attack:ammoCost, fire_modes:collectFireModes(ammoCost), magazines:[], shell_stocks:[], loaded_count:0});
      if (rt==='shell') {
        body.mag_capacity = num('shellCapacity',2);
        body.magazine_count = 0;
      } else {
        body.mag_capacity = num('magCapacity',30);
        body.magazine_count = num('magazineCount',0);
      }
...
async function quickReloadWeapon(itemId){
  const it = itemById(itemId);
  if (!it) return showToast('Оружие не найдено', 'error');
  if (it.reload_type === 'shell') return chooseLoadShells(itemId);
  const mag = cyberWeaponStoredMagazines(it)
    .sort((a,b) => Number(a.sort_order || a.id || 0) - Number(b.sort_order || b.id || 0))[0];
  if (!mag) return showToast('Нет запасного магазина', 'info');
  return requestReload(itemId, mag.id);
}
function openMagazineModal(itemId, magId){
  const it=itemById(itemId);
  const m=(it?.magazines||[]).find(x=>Number(x.id)===Number(magId))||{};
  const free = Math.max(0, Number(m.ammo_max || 0) - Number(m.ammo_current || 0));
  showModal(`<div class="modal-card compact"><button class="modal-close" onclick="closeModal()">×</button><div class="name">${esc(m.name||'Магазин')}</div><p class="muted">магазин от ${esc(it?.name || 'оружия')}</p><p><b>${esc(m.ammo_current||0)}/${esc(m.ammo_max||0)}</b> ${ammoTypePill(m.ammo_type || 'пустой',m.description)}</p>${m.description?`<p class="muted">${esc(m.description)}</p>`:''}<div class="row modal-actions"><button class="ok" ${free<=0?'disabled':''} onclick="chooseMagazineAmmo(${itemId}, ${magId})">Зарядить</button><button class="secondary" onclick="requestReload(${itemId}, ${magId})">Поставить в оружие</button><button class="secondary" onclick="closeModal()">Назад</button></div></div>`);
}
...
function chooseMagazineAmmo(itemId, magId){
  const it=itemById(itemId);
  const m=(it?.magazines||[]).find(x=>Number(x.id)===Number(magId));
  if(!it || !m) return;
  const free = Math.max(0, Number(m.ammo_max || 0) - Number(m.ammo_current || 0));
  if(free<=0) return showToast('Магазин уже полный','info');
  const currentType = Number(m.ammo_current || 0) > 0 ? String(m.ammo_type || '').trim() : '';
  const stocks=(it.shell_stocks||[]).filter(st => Number(st.quantity||0)>0 && (!currentType || currentType==='пустой' || String(st.ammo_type||'')===currentType));
  const rows=stocks.map(st=>{
    const max = Math.max(1, Math.min(free, Number(st.quantity||0)));
    return `<div class="mag-select shell-load-option"><div class="shell-load-info"><span class="mag-ammo"><b>${esc(st.quantity)}</b> шт.</span>${ammoTypePill(st.ammo_type,st.description)}${st.description?`<small>${esc(st.description)}</small>`:''}</div><div class="shell-load-controls"><input id="magLoadCount_${st.id}" type="number" min="1" max="${max}" value="${max}"><button class="ok small" onclick="requestChargeMagazine(${itemId}, ${magId}, ${st.id}, ${free})">Зарядить</button></div></div>`;
  }).join('') || '<div class="muted">Нет подходящих патронов. Создай патроны через ⚙ у оружия.</div>';
  showModal(`<div class="modal-card compact"><button class="modal-close" onclick="closeModal()">×</button><div class="name">Зарядить магазин</div><p class="muted">${esc(m.name||'Магазин')} · свободно мест: ${free}</p><div class="stack">${rows}</div><button class="secondary" onclick="openMagazineModal(${itemId}, ${magId})">Назад</button></div>`);
}
function chooseAmmoTargetMagazine(itemId, stockId){
  const it=itemById(itemId);
  const st=(it?.shell_stocks||[]).find(x=>Number(x.id)===Number(stockId));
  if(!it || !st) return;
  const mags=(it.magazines||[]).filter(m=>{
    if (Number(m.id) === Number(it.active_magazine_id)) return false;
    const free = Number(m.ammo_max||0) - Number(m.ammo_current||0);
    const cur = Number(m.ammo_current || 0) > 0 ? String(m.ammo_type||'').trim() : '';
    return free > 0 && (!cur || cur === 'пустой' || cur === String(st.ammo_type||''));
  });
  const rows=mags.map(m=>`<button class="mag-select" onclick="chooseMagazineAmmo(${itemId}, ${m.id})"><span class="mag-ammo"><b>${esc(m.ammo_current||0)}</b>/${esc(m.ammo_max||0)}</span>${ammoTypePill(m.ammo_type||'пустой', m.description||'')}<small>${esc(m.name||'Магазин')}</small></button>`).join('') || '<div class="muted">Нет пустого или совместимого магазина.</div>';
  showModal(`<div class="modal-card compact"><button class="modal-close" onclick="closeModal()">×</button><div class="name">Куда зарядить ${esc(st.ammo_type||'патроны')}?</div><div class="stack">${rows}</div><button class="secondary" onclick="closeModal()">Отмена</button></div>`);
}
async function requestChargeMagazine(itemId, magId, stockId, free){
  const count=Number(val(`magLoadCount_${stockId}`) || 0);
  if(!count) return showToast('Укажи количество','error');
  return sendInventoryRequest(`/api/inventory/items/${itemId}/refill-request`, {magazine_id:magId, stock_id:stockId, amount:Math.min(count, Math.max(1, Number(free||1)))});
}
function openMagazineCreateModal(itemId){
  const it=itemById(itemId)||{};
  showModal(`<div class="modal-card compact"><button class="modal-close" onclick="closeModal()">×</button><div class="name">➕ Магазин для ${esc(it.name||'оружия')}</div><label>Название магазина<input id="newMagName" placeholder="Например: основной, запасной"></label><label>Максимум патронов<input id="newMagMax" type="number" min="1" value="${esc(it.mag_capacity || 30)}"></label><label>Описание<textarea id="newMagDesc" placeholder="Необязательно"></textarea></label><div class="row modal-actions"><button class="ok" onclick="createMagazine(${itemId})">Создать пустой</button><button class="secondary" onclick="closeModal()">Отмена</button></div></div>`);
}
async function createMagazine(itemId){
  try{
    const maxAmmo=num('newMagMax',1);
    await api(`/api/inventory/items/${itemId}/magazines`, {method:'POST', body:{name:val('newMagName'), ammo_max:maxAmmo, ammo_current:0, ammo_type:'пустой', description:val('newMagDesc')}});
    state.campaignState=await api(`/api/campaigns/${state.currentCampaignId}/state`);
...
function openShellStockModal(itemId, stockId){
  const it=itemById(itemId);
  const st=(it?.shell_stocks||[]).find(x=>Number(x.id)===Number(stockId))||{};
  const action = it?.reload_type === 'shell'
    ? `<button class="ok" onclick="chooseLoadShells(${itemId})">Зарядить оружие</button>`
    : `<button class="ok" onclick="chooseAmmoTargetMagazine(${itemId}, ${stockId})">Зарядить магазин</button>`;
  showModal(`<div class="modal-card compact"><button class="modal-close" onclick="closeModal()">×</button><div class="name">${esc(st.ammo_type||'Патроны')}</div><p class="muted">патроны от ${esc(it?.name || 'оружия')}</p><p>В стопке: <b>${esc(st.quantity||0)}</b></p>${st.description?`<p class="muted">${esc(st.description)}</p>`:''}<div class="row modal-actions">${action}<button onclick="requestRefillShellStock(${itemId}, ${stockId})">Запросить пополнение</button><button class="danger" onclick="deleteShellStock(${itemId}, ${stockId})">Удалить стопку</button><button class="secondary" onclick="closeModal()">Назад</button></div></div>`);
}
async function deleteShellStock(itemId, stockId){
  if(!confirm('Удалить эту стопку патронов?')) return;
  try{
    const out = await api(`/api/inventory/items/${itemId}/shell-stocks/${stockId}`, {method:'DELETE'});
    const characterId = out.inventory?.[0]?.character_id || itemById(itemId)?.character_id || 0;
    if(characterId && out.inventory) replaceInventoryForCharacter(characterId, out.inventory);
    else state.campaignState=await api(`/api/campaigns/${state.currentCampaignId}/state`);
    closeModal(); showToast('Патроны удалены','info'); renderCampaign();
  }catch(e){ showToast(e.message,'error'); }
}
...
function openShellStockCreateModal(itemId){
  const it=itemById(itemId)||{};
  showModal(`<div class="modal-card compact"><button class="modal-close" onclick="closeModal()">×</button><div class="name">➕ Патроны для ${esc(it.name||'оружия')}</div><label>Название/тип патронов<input id="newStockAmmoType" placeholder="Например: обычные, бронебойные, дробь"></label><label>Количество<input id="newStockQty" type="number" min="0" value="30"></label><label>Описание<textarea id="newStockDesc" placeholder="Необязательно"></textarea></label><div class="row modal-actions"><button class="ok" onclick="createShellStock(${itemId})">Создать</button><button class="secondary" onclick="closeModal()">Отмена</button></div></div>`);
}
...
function renderInventoryRequests() {
  ...
  const label = (r)=>({reload_weapon:'смена магазина', refill_magazine:'зарядка магазина', load_shells:'зарядка оружия', refill_shell_stock:'пополнение патронов'}[r.request_type]||r.request_type);
  const requestDetails = (r)=>{
    ...
    if(r.request_type==='refill_magazine'){
      ...
      if(st) return `Зарядить ${m ? `${m.name || 'магазин'} ${m.ammo_current}/${m.ammo_max}` : 'магазин'}: ${p.amount||1} × ${st.ammo_type || 'патроны'} · в запасе ${st.quantity}`;
...
function previewAchievementIcon() {
  ...
  const file = input?.files?.[0];
  if (!file || !box) return;
  input.dataset.uploadId = createUploadId('achievement-icon');
  const url = URL.createObjectURL(file);
...
async function uploadAchievementIconIfNeeded() {
  ...
  const uploadId = input.dataset.uploadId || createUploadId('achievement-icon');
  input.dataset.uploadId = uploadId;
  const out = await apiUpload(`/api/campaigns/${state.currentCampaignId}/achievement-icon`, 'achIconFile', uploadId);
...
const customFramePreviewState = {url:'', uploadId:'', saving:false, scale:1.55, x:0, y:0, dragging:false, startX:0, startY:0, baseX:0, baseY:0};
...
function prepareCustomFramePreview() {
  ...
  customFramePreviewState.uploadId = createUploadId('custom-frame');
  customFramePreviewState.saving = false;
...
async function uploadCustomFrame() {
  if (customFramePreviewState.saving) return;
  customFramePreviewState.saving = true;
  const uploadButton = document.getElementById('uploadCustomFrameBtn');
  if (uploadButton) { uploadButton.disabled = true; uploadButton.textContent = 'Загрузка…'; }
  try {
    ...
    const uploadId = customFramePreviewState.uploadId || createUploadId('custom-frame');
    customFramePreviewState.uploadId = uploadId;
    const out = await apiUploadForm(`/api/campaigns/${state.currentCampaignId}/custom-frames`, 'customFrameFile', {name, description: '', rarity: 'unique', frame_scale, frame_offset_x, frame_offset_y}, uploadId);
    ...
    customFramePreviewState.saving = false;
    endFileInteraction();
    if (uploadButton) { uploadButton.disabled = false; uploadButton.textContent = 'Загрузить рамку'; }
    const el=document.getElementById('customFrameOut'); if(el) el.innerHTML=msg(e.message,'error'); else alert(e.message);
  }
}
...
function closeModal(){ const m=document.getElementById('modal'); if(m) m.remove(); endFileInteraction(); }
